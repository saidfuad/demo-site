  ...
  return $decimal;
   ...
   ORDER BY distance LIMIT 1";
	...
	function getAddress($lat, $lng) {
		...
		$query = "SELECT address FROM itms_geodata WHERE latitude = '$lat' AND longitude = '$lng' ORDER BY id DESC LIMIT 0,1";
		$res = mysql_query($query) or die(mysql_error().":".$query);
	...
	function log_raw_data($device, $data) {
		...
		$rawsql = "INSERT INTO itms_raw_data (device_id, raw_data, add_uid, add_date) VALUES ('".$device."','".$data."','1','".date(DATE_TIME)."')";
	...
	function area_in_out($device_id, $assets_id, $assets_name, $nick_name, $driver_name, $driver_mobile, $longitude_x, $latitude_y, $current_speed, $ist){
		...
		$sqlP = "SELECT DISTINCT (am.polyid) AS area_id, am.out_alert, am.in_alert, am.speed_value, am.speed_unit, 
							am.email_alert as email_alert, am.sms_alert as sms_alert, um.user_id, um.first_name, um.username, 
								um.mobile_number, um.language, um.email_address, um.email_alert as user_email_alert, 
									um.sms_alert as user_sms_alert, (SELECT group_concat(mobile_no) as mobile_no 
										FROM addressbook where find_in_set(area_id, am.addressbook_ids)) as addressbook_mobile 
											FROM itms_areas am LEFT JOIN itms_users um ON um.user_id = am.Audit_Enter_uid WHERE FIND_IN_SET( $assets_id, deviceid ) and am.Audit_Del_Dt is null and am.Audit_Status = 1";
	...
	function zone_in_out($device_id, $assets_id, $assets_name, $nick_name, $driver_name, $driver_mobile, $longitude_x, $latitude_y, $current_speed, $ist){
		...
		while($rowP = mysql_fetch_array($rsP)){
			...
			$sql = "SELECT * FROM itms_landmarks_areas WHERE polyid = $area_id";
			$rs = mysql_query($sql) or die("Failed to Execute, SQL : $sql, Error : " . mysql_error());
			$vertices_x = array();
			$vertices_y = array();
			while($row = mysql_fetch_array($rs)){
	...
	function checkLandmark($device_id, $assets_id, $assets_name, $nick_name, $driver_name, $driver_mobile, $lati, $longi, $current_speed, $ist, $odometer){
		...
		$sqlP = "SELECT lm.*, um.first_name, um.mobile_number, um.user_id, um.language, um.email_address, 
					um.email_alert as user_email_alert, um.sms_alert as user_sms_alert, um.alert_start_time, 
						um.alert_stop_time, (SELECT group_concat(mobile_no) as mobile_no FROM addressbook where 
								find_in_set(id, lm.addressbook_ids)) as addressbook_mobile FROM itms_landmarks lm 
					left join itms_users um on um.user_id = lm.add_uid WHERE FIND_IN_SET( $assets_id, lm.device_ids ) 
		...
		while($row = mysql_fetch_array($rs)){
			...
			if($distanceFromLandmark < $distance_value){	//"Device is near to Landmark"
				...
				$checkLast = "SELECT landmark_id, in_out FROM itms_landmarks_log WHERE device_id = $assets_id and landmark_id = $landmark_id order by id desc limit 1";
				...
				if($inOutStatus == 'out'){	
					...
					$checkLast1 = "select landmark_id, odometer FROM itms_landmarks_log where device_id = $assets_id order by id desc limit 1";
				...
				$checkLast = "select id FROM itms_landmarks_log where device_id = $assets_id and landmark_id = $landmark_id and in_out = 'in' order by id desc limit 1";
				$checkRs = mysql_query($checkLast) or die(mysql_error().":".$checkLast);
				...
				if(mysql_num_rows($checkRs) > 0){
					...
					if($dealer_code != ""){
						...
						$sub_sql = "select * from user_assets_map where user_id = (select user_id from itms_users where username = '$dealer_code')";
						$sub_rs = mysql_query($sub_sql) or die(mysql_error().":".$sub_sql);
						if(mysql_num_rows($tRs) > 0){
			...
			if($alert_before_landmark!=""){
				...
				if($distanceFromLandmark < $alert_before_landmark){	//"Device is near to Landmark"
					$checkLast = "select landmark_id FROM itms_landmarks_distance_log where device_id = $assets_id order by id desc limit 1";
					...
					if(($lastLandmarkId != $landmark_id) || ($numRows==0)){
						...
						// Dear $dealer_fname, 
						$smsText = "$assets_name ($driver_name, $driver_mobile) is $distanceText away FROM itms_landmarks $landmark_name, ".date(DISP_TIME, strtotime($ist));
	...
	function checkSpeed($device_id, $assets_id, $assets_name, $nick_name, $driver_name, $driver_mobile, $max, $current_speed, $ist, $lati, $longi) {
		...
		if($current_speed > $max && $max != "" && $max != 0){
			...
			if($minutes > 10){
				...
				$us_sql = "SELECT um.first_name, um.mobile_number, um.user_id, um.language, um.email_address, um.email_alert as user_email_alert, um.sms_alert as user_sms_alert, um.alert_start_time, um.alert_stop_time, um.ignition_on_alert, um.ignition_off_alert FROM user_assets_map lm left join itms_users um on um.user_id = lm.user_id WHERE FIND_IN_SET($assets_id, lm.assets_ids) and lm.del_date is null and lm.status = 1";
				// writeLog($us_sql);
	...
	function checkIgnitionOnSpeedOff($unit_no, $assets_id, $assets_name, $nick_name, $driver_name, $ignition, $speed, $last_speed, $latitude, $longitude, $x_address, $current)
   {
	...
	/*Added by Ashwini 12-9-2014 for below new VTS alerts :
	 ...
	 function new_vts_alerts($device,$reason_text, $assets_id, $assets_name, $nick_name, $latitude_x, $longitude_y, $x_address, $current){
            ...
            $us_sql = "SELECT um.first_name, um.mobile_number, um.user_id, um.language, um.email_address, um.email_alert as user_email_alert, um.sms_alert as user_sms_alert FROM user_assets_map lm left join itms_users um on um.user_id = lm.user_id WHERE FIND_IN_SET($assets_id, lm.assets_ids) and lm.del_date is null and lm.status = 1";
...
*/ 
	...
	function startStopTrip($device_id, $assets_id, $assets_name, $nick_name, $driver_name, $lati, $longi, $current_speed, $ist, $distance_travelled){
		...
		global $dts;
		$sqlP = "SELECT trip.*, um.user_id, um.first_name, um.mobile_number, um.language, um.email_address, um.email_alert as user_email_alert, um.sms_alert as user_sms_alert, am.driver_name, am.km_reading FROM tbl_routes trip LEFT JOIN itms_users um ON um.user_id = trip.add_uid left join assests_master am on am.current_trip = trip.id WHERE am.id = $assets_id and trip.del_date IS NULL AND trip.status = 1";
		...
		if(mysql_num_rows($rsP) > 0){
			...
			$sqlL = "SELECT * FROM itms_landmarks WHERE id = $start_point";
			...
			if($distanceFromLandmark > $distance_value){ 
				...
				if($trip_start_alert == true){
					...
					/*$sql = "select um.first_name, um.mobile_number, um.email_address, um.language, um.email_alert, um.sms_alert from itms_users um left join user_assets_map uam on uam.user_id = um.user_id where FIND_IN_SET( $assets_id, uam.assets_ids ) and um.del_date is null and um.status = 1 and um.user_id";
					$rs = mysql_query($sql) or die("Failed to Execute, SQL : $sql, Error : " . mysql_error());
					while($row = mysql_fetch_array($rs)){
						$fname 					= $row['first_name'];
						$mobile 				= $row['mobile_number'];
			...
			if($isCheckForLastPoint == true){
				...
				$sqlL = "SELECT * FROM itms_landmarks WHERE id = $end_point";
				$rs = mysql_query($sqlL);
				$row = mysql_fetch_array($rs);
		...
		if($current_trip != ""){
			$sqlP = "SELECT trip.*, um.first_name, um.mobile_number, um.user_id, um.language, um.email_address, um.email_alert as user_email_alert, um.sms_alert as user_sms_alert FROM tbl_routes trip left join itms_users um on um.user_id = trip.userid WHERE trip.del_date is null and trip.status = 1 and trip.id = $current_trip";

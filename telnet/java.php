<?php
	...
	//imei:359710048219167,tracker,1408250529,,F,235956.000,A,1849.8088,N,07422.3225,E,0.00,0,,0,0,0.11%,,
	...
	list($message_start, $unit_no, $msg_serial_no, $reason, $command_key, $command_key_value, 
			$ignition, $power_cut, $box_open, $msg_key, $odometer, $speed, $sat_mode, $gps_fixed, 
				$latitude, $longitude, $altitude, $direction, $time, $date, $gsm_strength, $gsm_register, 
					$gprs_register, $server_avail, $in_batt, $ext_batt_volt, $digital_io, $analog_in_1, 
						$analog_in_2, $analog_in_3, $analog_in_4, $hw_version, $sw_version, $data_type, $rfid) 
		= @explode(",", $input_data[1]);
	...
	if(count($input_data) > 2){
		list($message_start, $unit_no, $msg_serial_no, $reason, $command_key, $command_key_value, $ignition, 
				$power_cut, $box_open, $msg_key, $odometer, $speed, $sat_mode, $gps_fixed, $latitude, $longitude, 
					$altitude, $direction, $time, $date, $gsm_strength, $gsm_register, $gprs_register, $server_avail, 
						$in_batt, $ext_batt_volt, $digital_io, $analog_in_1, $analog_in_2, $analog_in_3, $analog_in_4, 
							$hw_version, $sw_version, $data_type, $rfid) 
			= @explode(",", $input_data[1]);

		global $device_id_j;
		$device_id_j=$unit_no;
		
		$aSql = "SELECT am.id, tm.time_zone, um.alert_stop_time, um.alert_start_time, um.from_date, um.to_date 
					FROM itms_assets am 
					left join itms_users um on um.user_id = am.add_uid 
					left join timezone tm on tm.diff_from_gmt = um.timezone 
					WHERE am.device_id = '".addslashes($unit_no)."' AND am.del_date IS NULL AND am.status = 1";

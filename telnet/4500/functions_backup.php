<?php
	...
	function send_email_message ($add_company_id, $device_id, $to, $reason, $subj, $message) {
		...
		if ($mail->Send()) {
			...
			if ($log_saved) {
				print_r("  [Email logged successfully]\n");
			} else {
				print_r("  [Failed to log email]\n");
			}
	...
	function checkLandmark($device_id, $assets_id, $driver_id,$assets_name, $nick_name, $driver_name, $driver_mobile, 
									$lati, $longi, $current_speed, $ist, $odometer, $sms_conts, $email_conts, $trip_id, $client_phone, $client_email) {
			...
			if($distanceFromLandmark < $distance_value){	//"Device is near to Landmark"
				...
				if($inOutStatus == 'out'){
					...
					if ($trip_id!=null || $trip_id!="") {
						track_trip($trip_id, $client_email, $client_phone, $landmark_name);
					}
	...
	function checkDestination($company_id, $device_id, $asset_id, $driver_id, $trip_id, $assets_name, $nick_name, 
		...
		if ($distance < $distance_to_alert) {
			print_r(" Asset near destination location...\n\n");

			print_r("[ tracking destination ]\n");
			track_destination($company_id, $device_id, $trip_id, $client_phone, $client_email, $x_address);
			...
			if ($countD < 1) {
				print_r("[ Updating near destination log ]\n");
				...
				$reason = "Destination Location";
				$message= "Destination Alert! $assets_name (Driver: $driver_name - $driver_phone ) Arrived at destination location: $client_address on $hdate";
				$res_alert = "";
			...
			if(mysql_num_rows($res) > 0){
				...
				if ($destination_in == 1 && $destination_out ==0) {
					print_r("[ Asset departing destination location ]\n");
					$query = "UPDATE itms_trips SET destination_out = 1 WHERE trip_id ='".$trip_id."'";
					$res = mysql_query($query) or die(mysql_error().":".$query);

					$reason = "Destination Location";
					$message= "Destination Alert! $assets_name (Driver: $driver_name - $driver_phone ) Departed destination location: $client_address on $hdate";
					$res_alert = "";
					update_alerts($company_id, $reason, $asset_id, $driver_id, $message, $driver_phone, $client_address, $x_address, $latitude, $longitude, $reason, $ist, $res_alert);

					$to = $emailRecievers;						
					$subj = "Destination departed : $client_address - $assets_name ";
	...
	function track_destination($company_id, $device_id, $trip_id, $client_phone, $client_email, $x_address) {
		$query = "SELECT * FROM itms_trips WHERE trip_id = '".$trip_id."'";
		$res = mysql_query($query) or die(mysql_error().":".$query);
		...
		if(mysql_num_rows($res) > 0){
			...
			if ($destination_in == 0 && $pickup_out==1) {
				print_r("[ Arriving at destination ]\n");
				$status = "Arriving at delivery location : ".$destination_address;

				$query = "UPDATE itms_trips SET destination_in = 1 WHERE trip_id ='".$trip_id."'";
				$res = mysql_query($query) or die(mysql_error().":".$query);

				$uhist = update_tracking_history($tracking_no, $status, $x_address);

				$reason = "Arriving at destination location";
				$notify_subj = "ITMS Pickup notification : Tracking Number - $tracking_no";
				$notify_message = "Dear Client, Our vehicle has arrived at delivery location : $destination_address";
	...
	function track_trip ($trip_id, $client_phone, $client_email, $landmark) {
		$query = "SELECT * FROM itms_trips WHERE trip_id = '".$trip_id."'";
		$res = mysql_query($query) or die(mysql_error().":".$query);
		...
		if(mysql_num_rows($res) > 0){
			...
			if ($destination_out != 1) {
				if ($pickup_in == 0) {
					print_r("[ Heading to pickup location ]\n");
					$status = "Heading to pickup location : ".$pickup_address;
				} else if ($pickup_in == 1 && $pickup_out == 1 && $destination_in ==0) {
					print_r("[ heading to delivery location ]\n");
					$status = "Heading to delivery location : ".$pickup_address;
				}

				$uhist = update_tracking_history($tracking_no, $status, $landmark);
			}
	...
	function update_tracking_history ($tracking_no, $status, $location) {
		print_r("\nUpdating tracking history..\n");				
				
		$date = gmdate("Y-m-d H:i:s");

		$query = "INSERT INTO itms_freight_tracking (tracking_no, status, location, date_time)
						VALUES ('".$tracking_no."', '".$status."', '".$location."', '".$date."')";
		$res = mysql_query($query) or die(mysql_error().":".$query);

		if ($res) {
			print_r(">> Tracking details updated Successfully\n");
		} else {
			print_r(">> Failed to update Tracking details\n");
		}

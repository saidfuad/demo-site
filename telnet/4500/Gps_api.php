...
//----------------------------------------------//
	...
	$input = $_REQUEST['gps'];
        $last_device_ip = $_POST['device_ip'];
	$output = '';
	$is_insert = true;
	$current_area = '';
	$current_landmark = '';
	...
	print_r("Direction Value: $dir ($orientation degrees from North)\n");
	...
	$aSql = "SELECT am.*, ip.*, iu.mobile_number as add_user_phone, iu.email_address as add_user_email, iu.user_id as add_user_id,
						...
						(select group_concat(user_id) from itms_assigned_groups where assets_group_id = am.assets_group_id) AS alert_users, it.end_lat, it.end_lng, it.destination_address, icm.email AS client_email, icm.phone_no AS client_phone, it.distance_to_alert
					FROM itms_assets am 
	...
	if((intval($latitude) != 0 && intval($longitude) != 0)) {
		...
		checkLandmark(addslashes($device_id), $asset_id, $driver_id,$assets_name, $nick_name, $driver_name, $driver_phone,
							addslashes($latitude), addslashes($longitude), $speed, $ist, $odomVal,$sms_conts,$email_conts);
        ...
        if ($countD) {
			...
			$duration = 0;
		  	...
		  	$sql = "INSERT INTO itms_last_gps_point 
		  				(company_id,device_ip, asset_id, driver_id,latitude, longitude, add_date, speed, 
		  					device_id, gps_availability, dt, tm, ignition, overspeed, max_speed_limit, 
				  				stop_duration, direction, angle_dir, address, command_key) 
					VALUES 

						('".$add_company_id."','".$last_device_ip."', '".$asset_id."', '".$driver_id."','".addslashes($latitude)."', '".addslashes($longitude)."',
							'".addslashes($ist)."', '".addslashes($speed)."', '".addslashes($device_id)."', 
									'".addslashes($gps_fixed)."', '".addslashes($ist)."', '".addslashes($time)."', 
										'".addslashes($ignition)."', '".addslashes($overspeed)."', '".addslashes($max_speed)."',
											'".addslashes($duration)."' , '".addslashes($dir)."','".addslashes($orientation)."', '".addslashes($x_address)."', '".addslashes($command)."')";

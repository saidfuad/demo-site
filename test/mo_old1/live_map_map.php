...
<?php
	...
	$query ="select * from assests_master am where am.status=1 AND am.del_date is null AND find_in_set(am.id, (SELECT assets_ids FROM user_assets_map where user_id = $user)) and am.device_id=".$_REQUEST['device']; 
...
function AddressShowHide(controlDiv, map){
  ...
  lblOne.innerHTML='Address OFF';
  controlUI.appendChild(lblOne);
  google.maps.event.addDomListener(controlUI, 'click', function() {
	if(lblOne.innerHTML=="Address OFF")
	{
...
window.addEventListener("resize", function() {
	...
	<div>
			<div style='float:left;height: 23px;vertical-align: middle;padding-top: 9px;padding-right: 6px;'>Refresh After</div>

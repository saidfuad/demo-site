...
//imei:359710041529935,tracker,1408250529,,F,235956.000,A,1849.8088,N,07422.3225,E,0.00,0,,0,0,0.11%,,; :Data Received
	...
	list($year, $month, $day, $hour, $min) = array(substr($date_time,0,2), substr($date_time,2,2), substr($date_time,4,2), substr($date_time,6,2), substr($date_time,8,2));
  ...
  if(strtolower($reason_text) == 'acc on'){
	...
	$query = "SELECT ignition,add_date,lati,longi,address FROM tbl_last_point WHERE device_id = '".addslashes($device)."'";
		...
		$old_latitude= $arr_row['lati'];
		$old_longitude = $arr_row['longi'];
	 ...
	 if ((($old_latitude == $new_latitude  && $old_longitude == $new_longitude ) || ($old_address == $new_address)) && $last_lignition == 0){
	  ...
	  $lSql = "UPDATE tbl_last_point set lati='".addslashes($latitude)."', longi='".addslashes($longitude)."', add_date='".addslashes($g_dt_tm)."', speed='".addslashes($speed)."', old_speed = '".$lastSpeed."', gps='".addslashes($gps_fixed)."', dt='".addslashes($g_date)."', tm='".addslashes($g_time)."', altitude='".addslashes($altitude)."', gsm_strength='".addslashes($gsm_strength)."', angle_dir='".addslashes($direction)."', address='".addslashes($x_address)."', odometer = '".addslashes($odomVal)."', reason_text = '".addslashes($reason_text)."', input_data = '".$input."', sat_mode = '".addslashes($sat_mode)."', gps_fixed = '".addslashes($gps_fixed)."', ext_batt_volt = '".addslashes($ext_batt_volt)."', ignition = '".addslashes($ignition)."' WHERE device_id = '".addslashes($device)."'";
	...
	if((intval($latitude) != 0 && intval($longitude) != 0)) {
		...
		$sql = "SELECT id, add_uid, assets_name from assests_master where device_id = '$device' limit 1";
		$rs = mysql_query($sql);
		...
		$aSql = "SELECT am.id, am.add_uid, am.assets_name, am.assets_friendly_nm, am.driver_name, am.max_speed_limit, am.current_trip, am.km_reading,  tm.time_zone, um.sms_enable, um.first_name, um.user_id, um.language, um.max_stop_time, um.mobile_number, um.email_address, um.email_alert, um.sms_alert, um.alert_stop_time, um.alert_start_time, um.ignition_on_alert, um.ignition_off_alert, um.ignition_on_speed_off_minutes, um.ignition_off_speed_on_minutes, um.from_date, um.to_date FROM assests_master am left join tbl_users um on um.user_id = am.add_uid left join timezone tm on tm.diff_from_gmt = um.timezone WHERE am.device_id = '".addslashes($device)."' AND am.del_date IS NULL AND am.status = 1";
		...
		area_in_out(addslashes($device), $assets_id, $assets_name, $nick_name, $driver_name, $driver_mobile, addslashes($latitude),addslashes($longitude), $speed, $ist);

		checkLandmark(addslashes($device), $assets_id, $assets_name, $nick_name, $driver_name, $driver_mobile, addslashes($latitude), addslashes($longitude), $speed, $ist, $odomVal);

		//panic_status($alarm_status, $device, $assets_id, $assets_name, $longitude, $latitude, $ist);

		$sql = "INSERT INTO tbl_track SET assets_id = '$assets_id', lati = '".addslashes($latitude)."', longi = '".addslashes($longitude)."', add_date = '".addslashes($g_dt_tm)."', speed = '".addslashes($speed)."', device_id = '".addslashes($device)."', gps = '".addslashes($gps_fixed)."', dt = '".date(DATE_TIME)."', tm = '".addslashes($g_time)."', angle_dir = '".addslashes($direction)."', address = '".addslashes($x_address)."', odometer = '".addslashes($odomVal)."', reason = '".addslashes($eventCode)."', reason_text = '".addslashes($reason_text)."', in_batt = '".addslashes($in_batt)."', ext_batt_volt = '".addslashes($ext_batt)."', current_area_id = '$current_area_id', current_landmark_id = '$current_landmark_id', data_type = 0, distance_by_latlong = '".$distance_by_latlong."'";
		...
		$sql = "SELECT address, add_date, speed, odometer, lati, longi FROM tbl_last_point WHERE device_id = '".addslashes($device)."'";
		$rs = mysql_query($sql);
		...
		if(mysql_num_rows($rs) > 0){
			...
			$lSql = "UPDATE tbl_last_point set lati='".addslashes($latitude)."', longi='".addslashes($longitude)."', add_date='".addslashes($g_dt_tm)."', speed='".addslashes($speed)."', old_speed = '".$lastSpeed."', gps='".addslashes($gps_fixed)."', dt='".addslashes($g_date)."', tm='".addslashes($g_time)."', altitude='".addslashes($altitude)."', gsm_strength='".addslashes($gsm_strength)."', angle_dir='".addslashes($direction)."', address='".addslashes($x_address)."', odometer = '".addslashes($odomVal)."', reason_text = '".addslashes($reason_text)."', input_data = '".$input."', sat_mode = '".addslashes($sat_mode)."', gps_fixed = '".addslashes($gps_fixed)."', ext_batt_volt = '".addslashes($ext_batt_volt)."'";
...
*/
		...
		if($odomVal > 0){
			...
			if($distance_travelled > 0) {
				$update = "UPDATE assests_master SET km_reading = (km_reading + ".floatval($distance_travelled).") WHERE id = ".$assets_id;
				mysql_query($update);
			}
			...
			if($last_odometer != "" && $odomVal > $last_odometer){
				...
				if(mysql_num_rows($rs) > 0){
					...
					$query = "UPDATE distance_master SET distance = ($odomVal - first_reading), current_reading = '".addslashes($odomVal)."' $up_sub WHERE assets_id = '".addslashes($assets_id)."' and add_date = '".$user_date."'";
					mysql_query($query);
				}else{
					...
					$query = "INSERT INTO distance_master (assets_id, add_date, first_reading, current_reading, distance) VALUES ('".addslashes($assets_id)."', '".$user_date."', '".addslashes($last_odometer)."', '".addslashes($odomVal)."', '$distance')";
					mysql_query($query);
				}
			}
		}
			...
			$update = "UPDATE assests_master SET km_reading = (km_reading + ".floatval($distance_by_latlong).") WHERE id = ".$assets_id;
			mysql_query($update);
			$sql = "SELECT distance FROM distance_master WHERE assets_id = '".addslashes($assets_id)."' AND add_date = '".$user_date."'";
			$rs = mysql_query($sql);
			if(mysql_num_rows($rs) > 0){
				//$query = "UPDATE distance_master SET distance = (distance + ".floatval($distance_by_latlong).") WHERE assets_id = ".$assets_id." and add_date = ".$g_dt_tm;
				$query = "UPDATE distance_master SET distance = (distance + ".floatval($distance_by_latlong).") WHERE assets_id = '".addslashes($assets_id)."' and add_date = '".$user_date."'";				
				mysql_query($query);
			}else{			
				$query = "INSERT INTO distance_master (assets_id, add_date, distance) VALUES ('".addslashes($assets_id)."', '".$user_date."', '$distance_by_latlong')";
				mysql_query($query);
			}

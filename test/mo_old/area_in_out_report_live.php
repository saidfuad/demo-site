...
<?php $device_id = $_REQUEST['device']; ?>
	<script type="text/javascript">
		$(document).live( "pagecreate", function() {
				...
				$("#datepick_area_in_out1").mobipick();
				$("#datepick_area_in_out").bind( "change", function() {
					var date = $( this ).val();
					var dateObject = $( this ).mobipick( "option", "date" );
				});	
				$("#datepick_area_in_out1").bind( "change", function() {
					var date = $( this ).val();
					var dateObject = $( this ).mobipick( "option", "date" );
				});	
		});
	...
	?>
	<center><h3><?php echo $lang['Area In/Out Report'].$names; ?></h3></center>
		...
		<!--	<table width='100%' id='table' border="1" style='font-size:12px !important'>
				...
				<?php
					...
					if(mysql_num_rows($res)<1)
					{
						...
						$row=mysql_fetch_array($res);
						echo "<div data-role='collapsible-set' data-theme='b' data-content-theme=''><div data-role='collapsible' data-collapsed=''><h3>".$row['device']."</h3><div data-role='fieldcontain'>";
						echo '<table data-role="table" id="table-custom-2" data-mode="columntoggle" class="ui-body-d ui-shadow table-stripe ui-responsive" data-column-btn-theme="b" data-column-btn-text="'.$lang['Columns to display...'].'" data-column-popup-theme="a"><thead><tr><th data-priority="1">'.$lang['Area Name'].' </th><th data-priority="1">'.$lang['Date Time'] .' </th><th data-priority="1" >'.$lang['status'] .' </th></tr></thead>';
						$footer="</tbody></table></div></div></div>";
					}
					$res =mysql_query($query) or die($query.mysql_error());
					while($row=mysql_fetch_array($res))
					{
						echo '<tr><td>'.$row['area'].'</td><td>'.date($_SESSION['date_format']." ".$_SESSION['time_format'],strtotime($row['date'])).'</td><td style="text-align:center">'.$row['status'].'</td></tr>';
					}
					echo $footer;
			?>
	<!--	</table>-->
				...
				<table style='width:100%;'>
					...
					<tr  align='center'>
						<td  align='right'><?php echo $lang['To Date']; ?>:</td>
  	                	<td><input type="text" name="date1" id="datepick_area_in_out1"   /></td>
					</tr>
					<?php 
						$user = $_SESSION['user_id'];
					...
					<tr align='center'>
						...
						<td>
							<input type="hidden" name="device" id="device" value="<?php echo $div_id
							; ?>"/>
						</td>
					</tr>
					<tr align='center'>

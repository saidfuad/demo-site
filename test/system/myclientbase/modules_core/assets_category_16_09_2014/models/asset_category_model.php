...
class Asset_category_model extends Model 
{
	...
	*/
	...
	function getAllData(){
		...
		$SQL = "SELECT cm.*,tm.assets_type_nm as t_name, aci.image_path as icon_path FROM assests_category_master as cm left join assests_type_master as tm on cm.assets_type_id=tm.id  left join assests_category_images aci on aci.id=cm.assets_cat_image  where cm.status=1 and cm.del_date is null";
		if($where != "")
			$SQL .= " AND $where";
			//die($SQL);

...
class Assets_command extends Admin_Controller {
	
	function __construct() {

		parent::__construct(TRUE);

		$this->load->model('asset_command_model','',TRUE);
		$this->load->model('form_model','',TRUE);
	}
	
	function index()
	{
		$this->load->view('assets_command_grid');
	}
	
	function loadData(){
		
		$data = $this->asset_command_model->getAllData(); 
	...
	//	echo $this->asset_category_model->delete_assets(); 
		$this->output->set_output($this->asset_command_model->delete_commands());
	}
...
//		print_r($data);
	...
	function form() {
		...
		if (!$this->form_model->validate()) {
			...
			$res['classcombo']=$this->asset_command_model->fillClassCombo(); 
			...
			if(uri_assoc('id')){
				...
				$this->asset_command_model->save($formdata, uri_assoc('id'));
			}
		}
	}
	...
	function chk_nm()
	{
		$rows = $this->asset_command_model->check_assets_command();

...
//		$this->load->view('all_points_report', $data);
	...
	function get_all_points_report(){
		...
		foreach($rows as $row) {
			...
			$html .= "<br>".$speed." KM";
			$html .= "<br>".$row['address'];
			$row['map'] = "<a href='#' onclick='view_map(\"$id\", \"$name\")'> <img src='".base_url()."assets/marker-images/mini-RED-BLANK.png'></a>";
...
*/	
	...
	function deleteLandmark(){
		...
		$this->load->model('home_model');
		$this->home_model->delete_landmark();
		exit;
	}
	function addLandmark(){
		$this->load->model('home_model', '',TRUE);
		$result = $this->home_model->add_landmark();
		//die($result);
		$this->output->set_output($result);
	}
	function updateLandmark(){
		$this->load->model('home_model');
		$res = $this->home_model->update_landmark();
		$responce['result'] = $res['result'];
		$responce['id'] = $res['insert_id'];
		$responce['msg'] = $res['msg'];
		//echo json_encode($responce);

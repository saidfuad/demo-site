...
class Home extends Admin_Controller {
	...
	function chat(){
		...
		if($action=='chatheartbeat'){
			$out = $this->home_model->chatheartbeat();
		}if($action=='closechat'){
			$out = $this->home_model->closechat();
		}
		if($action=='startchatsession'){
			$out = $this->home_model->startchatsession();
		}
		if($action=='sendChat'){
			$out = $this->home_model->sendChat();
		}
	...
	function index(){
		...
		$row = $this->home_model->getOnscreenAlertSettings();
		$data['onscreen_alert'] = $row->onscreen_alert;
	...
	function geofence(){
		...
		*/
		...
		if(count($rows)) {
			foreach ($rows as $row) {				
				$deviceGrp .= "<option value='".$row->id."'>".$row->assets_name."</option>";
			}
		}
		else {
			$deviceGrp .= "<option value=''>No Assets</option>";
		}
	...
	function assets_list()
	{
		...
		if(count($rows) > 0) {
			foreach ($rows as $row) {
				if($row->stop_from != ''){
					...
					// if($row->device_id == 390)
					// echo '<br/>'.gmdate('Y-m-d H:i:s').' - '.$row->stop_from;
		...
		if(count($rows) > 0) {
			foreach ($rows as $row) {
				...
				$c_id=$row->assets_category_id;
                $data1 = $this->home_model->get_catagory_image_id($c_id);
				foreach($data1->result_Array() as $row1)
					{
						$image_type =$row1['assets_cat_image'];
					}
		...
		if(count($rows) > 0) {
			foreach ($rows as $row) {
				...
				*/
				...
				$text .="Status: ";
				if($minutes_before < $this->session->userdata('network_timeout') && $row->speed > 10 && $minutes_before != ""){
						$status ="Running";
				}else if($minutes_before < $this->session->userdata('network_timeout') && $row->speed <= 10 && $row->ignition == 0 && $minutes_before != ""){
						$status ="Parked";
				}else if($minutes_before < $this->session->userdata('network_timeout')  && $row->speed <= 10 && $row->ignition == 1 && $minutes_before != ""){
						$status ="Idle";
...
//		$combo_s.="<option value='device_fault'>".$this->lang->line('device_fault')."</option>";
	...
	function assets_location(){
		...
		//$row1 = $this->home_model->assets_det();
		$rows = $rowss->result();
		$row = $rows[0];
		...
		$text ="<b>". $row->assets_name." (".$row->device_id.")</b><br>";
		...
		$seconds_before = ($row->beforeTime);
		 $text .="Status: ";
				if($seconds_before < $this->session->userdata('network_timeout') && $row->speed > 10 && $row->ignition == 1 && $seconds_before != ""){
						$status ="Running";
						
				}else if($seconds_before < $this->session->userdata('network_timeout') && $row->ignition == 0 && $seconds_before != ""){
						$status ="Parked";
						
				}else if($seconds_before < $this->session->userdata('network_timeout') && $row->speed <= 10 && $row->ignition == 1 && $seconds_before != ""){
						$status ="Idle";
...
//		$this->load->view('all_points_report', $data);
	...
	function get_all_points_report(){
		...
		foreach($rows as $row) {
			...
			$html .= "<br>".$speed." KM";
			$html .= "<br>".$row['address'];
			$row['map'] = "<a href='#' onclick='dashboard_view_map(\"$id\", \"$name\")'> <img src='".base_url()."assets/marker-images/mini-BLUE1-BLANK.png'></a>";

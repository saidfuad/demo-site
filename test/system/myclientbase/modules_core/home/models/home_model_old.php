...
class Home_model extends Model 
{
	...
	*/
	...
	public function getAssetsStatus($user)
	{
		
		$qry="SELECT ";
		//running
		if($this->session->userdata('usertype_id') == 1){
			$qry.=" (select count(am.id) from assests_master am left join tbl_last_point lm on lm.device_id = am.device_id where am.status=1 AND am.del_date is null AND (TIME_TO_SEC(TIMEDIFF( NOW( ) , lm.add_date)) <= 1200 and lm.speed > 0)) as Running,";
			
			$qry.=" (select count(am.id) from assests_master am left join tbl_last_point lm on lm.device_id = am.device_id where am.status=1 AND am.del_date is null AND (TIME_TO_SEC(TIMEDIFF( NOW( ) , lm.add_date)) <= 1200 and lm.speed = 0) ) as Parked,";
			
			$qry.=" (select count(am.id) from assests_master am left join tbl_last_point lm on lm.device_id = am.device_id where am.status=1 AND am.del_date is null AND (TIME_TO_SEC(TIMEDIFF( NOW( ) , lm.add_date)) between 1201 and 86399) ) as out_of_network,";
			
			$qry.="(select count(am.id) from assests_master am left join tbl_last_point lm on lm.device_id = am.device_id where am.status=1 AND am.del_date is null AND ((TIME_TO_SEC(TIMEDIFF( NOW( ) , lm.add_date))) >= 86400 or (lm.add_date is null))) as device_fault,";
			
			$qry.="(SELECT count(am.id) from assests_master am left join tbl_last_point lm on lm.device_id = am.device_id where am.status=1 AND am.del_date is null And find_in_set(am.id, (SELECT assets_ids FROM user_assets_map where user_id = ".$user."))) as total";
		}else{
			$qry.=" (select count(am.id) from assests_master am left join tbl_last_point lm on lm.device_id = am.device_id where am.status=1 AND am.del_date is null AND find_in_set(am.id, (SELECT assets_ids FROM user_assets_map where user_id = ".$user." )) AND (TIME_TO_SEC(TIMEDIFF( NOW( ) , lm.add_date)) <= 1200 and lm.speed > 0)) as Running,";
			
			$qry.=" (select count(am.id) from assests_master am left join tbl_last_point lm on lm.device_id = am.device_id where am.status=1 AND am.del_date is null AND find_in_set(am.id, (SELECT assets_ids FROM user_assets_map where user_id = ".$user." )) AND (TIME_TO_SEC(TIMEDIFF( NOW( ) , lm.add_date)) <= 1200 and lm.speed = 0) ) as Parked,";
			
			$qry.=" (select count(am.id) from assests_master am left join tbl_last_point lm on lm.device_id = am.device_id where am.status=1 AND am.del_date is null AND find_in_set(am.id, (SELECT assets_ids FROM user_assets_map where user_id = ".$user." )) AND (TIME_TO_SEC(TIMEDIFF( NOW( ) , lm.add_date)) between 1201 and 86399) ) as out_of_network,";
			
			$qry.="(select count(am.id) from assests_master am left join tbl_last_point lm on lm.device_id = am.device_id where am.status=1 AND am.del_date is null AND find_in_set(am.id, (SELECT assets_ids FROM user_assets_map where user_id = ".$user." )) AND ((TIME_TO_SEC(TIMEDIFF( NOW( ) , lm.add_date))) >= 86400 or (lm.add_date is null))) as device_fault,";
	...
	public function last_location($user) 
	{
		...
		elseif($rptsub == "u-"){
			...
			switch ($report) { case "running":
					$stsWhr = " AND (TIME_TO_SEC(TIMEDIFF(NOW( ), lm.add_date)) <= 1200 and lm.speed > 0)";	break;
				case "parked":
					$stsWhr = " AND TIME_TO_SEC(TIMEDIFF(NOW( ), lm.add_date)) <= 1200 and lm.speed = 0"; break;
				case "out_of_network":
					$stsWhr = " AND TIME_TO_SEC(TIMEDIFF(NOW( ), lm.add_date)) between 1201 and 86399";	break;
				case "device_fault":
					$stsWhr = " AND (TIME_TO_SEC(TIMEDIFF(NOW( ), lm.add_date)) >= 86400 or (lm.add_date is null))"; break;
			}
		}
		if($group != ""){
		...
		$qryFinal ="SELECT count(am.id) as total from assests_master am left join tbl_last_point lm on lm.device_id = am.device_id where am.status=1 AND am.del_date is null AND $sub $srch $stsWhr";
	...
	//	$report = 'running';
		...
		if($rptsub == "g-"){
			...
			switch ($report) { case "running":
					$stsWhr = " AND (TIME_TO_SEC(TIMEDIFF(CONVERT_TZ( NOW( ) , '+00:00', '".$this->session->userdata('diff_from_gmt')."' ) , lm.add_date)) <= 1200 and lm.speed > 0)";	break;
				case "parked":
					$stsWhr = " AND TIME_TO_SEC(TIMEDIFF(CONVERT_TZ( NOW( ) , '+00:00', '".$this->session->userdata('diff_from_gmt')."' ) , lm.add_date)) <= 1200 and lm.speed = 0"; break;
				case "out_of_network":
					$stsWhr = " AND TIME_TO_SEC(TIMEDIFF(CONVERT_TZ( NOW( ) , '+00:00', '".$this->session->userdata('diff_from_gmt')."' ) , lm.add_date)) between 1201 and 86399";	break;
				case "device_fault":
					$stsWhr = " AND (TIME_TO_SEC(TIMEDIFF(CONVERT_TZ( NOW( ) , '+00:00', '".$this->session->userdata('diff_from_gmt')."' ) , lm.add_date)) >= 86400 or (lm.add_date is null))"; break;
			}
		}
		/*elseif($report == "running"){
			$stsWhr = " AND (TIME_TO_SEC(TIMEDIFF(CONVERT_TZ( NOW( ) , '+00:00', '+05:30' ) , lm.add_date)) <= 1200 and lm.speed > 0)";
		...
		elseif($report == "out_of_network"){
			$stsWhr = " AND TIME_TO_SEC(TIMEDIFF(CONVERT_TZ( NOW( ) , '+00:00', '+05:30' ) , lm.add_date)) between 1201 and 86399";
		}
		elseif($report == "device_fault"){
			$stsWhr = " AND (TIME_TO_SEC(TIMEDIFF(CONVERT_TZ( NOW( ) , '+00:00', '+05:30' ) , lm.add_date)) >= 86400 or (lm.add_date is null))";
		}*/
		if($group != ""){
			$this->db->select("assets", FALSE);
			$this->db->where('id', $group);
	...
	public function get_subuser($user)
	{
		...
		$query = $this->db->query("select user_id, username, first_name, last_name from tbl_users where status=1 And del_date is null And (find_in_set('$today',display_day) or display_day='all') and admin_id = $user");
		return $query->result();	
	}
	...
	public function device_map($user) 
	{
		...
		$query = $this->db->query("SELECT lm.id, am.id as assets_id, am.assets_friendly_nm, am.assets_name, am.sim_number, am.driver_name, am.driver_mobile, am.driver_image, am.assets_image_path, am.device_id, CONVERT_TZ(lm.add_date,'+00:00','".$this->session->userdata('timezone')."') as add_date , TIME_TO_SEC(TIMEDIFF( NOW( ) , lm.add_date)) as beforeTime, lm.address, lm.lati, lm.longi, lm.speed, lm.ignition, im.icon_path, lm.fuel_percent, lm.fuel_liter, lm.fuel_time, lm.temperature, lm.angle_dir from assests_master am left join tbl_last_point lm on lm.device_id = am.device_id LEFT JOIN icon_master im ON im.id = am.icon_id where $sub");
...
s		foreach ($res->result() as $row)
	...
	{
		$device_ids = uri_assoc('id');
		$query = $this->db->query("select am.*, im.icon_path from assests_master am left join icon_master im on im.id = am.icon_id where am.id = $device_ids");
		return $query->row();
	}

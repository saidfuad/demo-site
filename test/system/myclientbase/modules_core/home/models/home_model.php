...
class Home_model extends Model 
{
	...
	*/
	...
	function startChatSession() {
		...
		$data = array("username" => $this->session->userdata('user_id'),
			"items" => '',
			"display_name" => trim($this->session->userdata('first_name')." ".$this->session->userdata('last_name'))
		);
	...
	public function get_stop_report() 
	{
		...
		if($count>0){
			foreach($result->result_array() as $row)
			{
				...
				//sanjaysinh jadeja
				$diff=strtotime($ignition_on)-strtotime($ignition_off); 
	...
	/*
	public function get_all_points_report() 
	{
		$this->table_name = "tbl_track";
		$date_format = $this->session->userdata('date_format');  
		$time_format = $this->session->userdata('time_format'); 
		$device = $_REQUEST['device'];
		$data = "<table style='border:1px solid !important' width=96% cellpadding='10' cellspacing='10'>";
		$data .= "<tr style='border:1px solid !important'><th width='70px' style='border:1px solid !important'>Datetime</th><th  width='100px' style='border:1px solid !important'>Status</th><th  width='100px' style='border:1px solid !important'>Address</th><th width='100px' style='border:1px solid !important'>Speed</th><th width='100px' style='border:1px solid !important'>View On Map </th></tr>";
				
		$SQL = "SELECT * FROM ".$this->table_name." WHERE assets_id = '$device'";
		...
		if($count>0){
			foreach($result->result_array() as $row)
			{
				...
				$speed = $row['speed'];
				$ignition = intval($row['ignition']);
				$status = "";
				
				if($ignition == 0) {
					$status = 'Parked';
				}else if($spee == 0 && $ignition == 1){
					$status = "Idle";
				...
				$lat = $row['lati'];
				$lng = $row['longi'];
				$html = date("$date_format $time_format", strtotime($add_date));
				$html .= "<br>".$speed." KM";
				$html .= "<br>".$row['address'];
				$rowactions = "<a href='#' onclick='viewAllpointonmap(\"$lat\", \"$lng\", \"$html\")'> <img src='".base_url()."assets/marker-images/mini-RED-BLANK.png'></a>";
				$data .="<tr align='center' style='border:1px solid !important'>";
				$data.="<td nowrap style='border:1px solid !important'>&nbsp;".date("$date_format $time_format", strtotime($add_date))."</td>"; 
				$data.="<td style='border:1px solid !important'>".$status."</td>"; 
				$data.="<td nowrap style='border:1px solid !important'>".$row['address']."</td>";
				$data.="<td style='border:1px solid !important'>".$row['speed']."</td>";
				$data.="<td style='border:1px solid !important'>".$rowactions."</td>";
				$data .="</tr>";
			}
			$data .= "</table>";
		}
	...
	*/
	...
	public function get_last_20_points_report() 
	{
		$date_format = $this->session->userdata('date_format');  
		$time_format = $this->session->userdata('time_format'); 
		$device = $_REQUEST['device'];
		
		//$SQL = "SELECT id, CONVERT_TZ(add_date,'+00:00','".$this->session->userdata('timezone')."') as add_date, address, ignition, speed, lati, longi FROM ".$this->table_name." WHERE assets_id = '$device' ORDER BY id DESC LIMIT 0, 20";

		$SQL = "SELECT lp.id, CONVERT_TZ(lp.add_date,'+00:00','".$this->session->userdata('timezone')."') as add_date, lp.address, lp.ignition, lp.speed, lp.lati, lp.longi, am.assets_name FROM ".$this->table_name." lp LEFT JOIN ".$this->tbl_assets." am ON am.id = lp.assets_id WHERE lp.assets_id = '$device' ORDER BY lp.id DESC LIMIT 0, 20";
	...
	public function get_area_in_out() 
	{
		...
		$SQL = "SELECT distinct(tm.id),tm.date_time as date, tm.inout_status as status, am.assets_name as device, ta.polyname as area FROM ".$this->table_name." as tm left join ".$this->tbl_assets." am on am.id = tm.device_id left join ".$this->tbl_area." as ta on tm.area_id=ta.polyid WHERE am.id in (select id from assests_master where find_in_set(id, (SELECT assets_ids FROM user_assets_map where user_id = $user)))";
		if($device){	//search by device
			$SQL .= " AND find_in_set(tm.device_id,'$device')";
		}
	...
	public function get_owners(){
		...
		$query = "SELECT id, owner FROM `assests_owner_master` WHERE status = '1' AND del_date is Null ORDER BY owner ASC ";
	...
	public function get_divisions(){
		...
		$query = "SELECT id, division FROM `assests_division_master` WHERE status= '1' AND del_date IS NULL ORDER BY division ASC ";
		$query = $this->db->query($query);
		return $query->result();
	...
	public function getAssetsStatus($user, $group)
	{
		...
		if($this->session->userdata('usertype_id') == 1 && $user == 1){
			...
			$qry.=" (select count(am.id) from assests_master am left join tbl_last_point lm on lm.device_id = am.device_id where am.status=1 AND lm.ignition = 0 AND am.del_date is null $group AND (TIME_TO_SEC(TIMEDIFF( NOW( ) , CONVERT_TZ(lm.add_date,'+00:00','".$this->session->userdata('timezone')."'))) < ".$this->session->userdata('network_timeout')." and lm.speed = 0) ) as Parked,";
			
			$qry.=" (select count(am.id) from assests_master am left join tbl_last_point lm on lm.device_id = am.device_id where am.status=1 AND lm.ignition = 1 AND am.del_date is null $group AND (TIME_TO_SEC(TIMEDIFF( NOW( ) , CONVERT_TZ(lm.add_date,'+00:00','".$this->session->userdata('timezone')."'))) < ".$this->session->userdata('network_timeout')." and lm.speed = 0) ) as Idle,";
			...
			$qry.=" (select count(am.id) from assests_master am left join tbl_last_point lm on lm.device_id = am.device_id where am.status=1 AND lm.ignition = 0 AND am.del_date is null $group AND find_in_set(am.id, (SELECT assets_ids FROM user_assets_map where user_id = ".$user." )) AND (TIME_TO_SEC(TIMEDIFF( NOW( ) , CONVERT_TZ(lm.add_date,'+00:00','".$this->session->userdata('timezone')."'))) < ".$this->session->userdata('network_timeout')." and lm.speed = 0) ) as Parked,";
			
			$qry.=" (select count(am.id) from assests_master am left join tbl_last_point lm on lm.device_id = am.device_id where am.status=1 AND lm.ignition = 1 AND am.del_date is null $group AND find_in_set(am.id, (SELECT assets_ids FROM user_assets_map where user_id = ".$user." )) AND (TIME_TO_SEC(TIMEDIFF( NOW( ) , CONVERT_TZ(lm.add_date,'+00:00','".$this->session->userdata('timezone')."'))) < ".$this->session->userdata('network_timeout')." and lm.speed = 0) ) as Idle,";
	...
	public function all_location($user) 
	{
		...
		$Fqry1="SELECT um.username, um.first_name, am.id as assets_id, am.assets_category_id, ad.division as assets_division, ao.owner as assets_owner, am.fuel_in_out_sensor, am.fuel_in_per_lit, am.fuel_in_company_name, am.fuel_in_product_code, am.fuel_out_per_lit, am.fuel_out_company_name, am.fuel_out_product_code, am.xyz_sensor, am.battery_size, am.device_id, am.assets_friendly_nm, am.max_fuel_liters, am.assets_name, am.driver_name, am.assets_image_path, am.driver_mobile, am.sim_number, am.driver_image, am.km_reading, am.eng_runtime, CONVERT_TZ(lm.add_date,'+00:00','".$this->session->userdata('timezone')."') as add_date, TIME_TO_SEC(TIMEDIFF( NOW( ), CONVERT_TZ(lm.add_date,'+00:00','".$this->session->userdata('timezone')."'))) as beforeTime, lm.address, lm.runtime, lm.lati, lm.longi, lm.speed, lm.old_speed, lm.ignition, im.icon_path, lm.current_area, lm.current_zone, lm.current_landmark, lm.cross_speed, lm.fuel_percent, lm.fuel_liter, CONVERT_TZ(lm.fuel_time,'+00:00','".$this->session->userdata('timezone')."') as fuel_time, lm.temperature, lm.reason_text, lm.reason, lm.command_key, lm.command_key_value, lm.msg_key, lm.sat_mode, lm.gps_fixed, lm.gsm_register, lm.gsm_strength, lm.gprs_register, lm.server_avail, lm.in_batt, lm.ext_batt_volt, lm.captured_image FROM assests_master am left join tbl_last_point lm on lm.device_id = am.device_id LEFT JOIN icon_master im ON im.id = am.icon_id LEFT JOIN tbl_users um on um.user_id = am.add_uid LEFT JOIN assests_owner_master ao ON ao.id = am.assets_owner LEFT JOIN assests_division_master ad ON ad.id = am.assets_division LEFT JOIN tbl_routes trip on trip.id = am.current_trip where am.status=1 AND am.del_date IS NULL $gsub ORDER BY am.assets_name";
		// echo $Fqry1;
		$query = $this->db->query($Fqry1, FALSE);
	...
	public function stop_duration($user) 
	{
		$page = $_POST['page'];
		$limit = $_POST['rows'];
		$reports = $_POST['report'];
		...
		// $Fqry1="SELECT sp.device_id, TIME_TO_SEC(TIMEDIFF(utc_timestamp(), MAX( sp.ignition_off )))/60 stop_from FROM tbl_stop_report sp LEFT JOIN assests_master am ON am.id = sp.device_id LEFT JOIN tbl_last_point lm ON lm.device_id = am.device_id WHERE lm.speed = 0 ".$gsub." ".$srch." ".$stsWhr." GROUP BY sp.device_id";
		
		// $Fqry1="SELECT sp.device_id, TIME_TO_SEC(UNIX_TIMESTAMP(NOW()) - UNIX_TIMESTAMP(MAX( sp.ignition_off )))/60 stop_from FROM tbl_stop_report sp LEFT JOIN assests_master am ON am.id = sp.device_id LEFT JOIN tbl_last_point lm ON lm.device_id = am.device_id WHERE lm.speed = 0 ".$gsub." ".$srch." ".$stsWhr." GROUP BY sp.device_id";
		
		$Fqry1="SELECT sp.device_id, MAX(sp.ignition_off) as stop_from FROM tbl_stop_report sp LEFT JOIN assests_master am ON am.id = sp.device_id LEFT JOIN tbl_last_point lm ON lm.device_id = am.device_id WHERE lm.speed = 0 ".$gsub." ".$srch." ".$stsWhr." GROUP BY sp.device_id";
	...
	{
		if($user != 1){
			$sql = "SELECT gm.id, gm.group_name, GROUP_CONCAT( am.id ) as assets FROM assests_master am LEFT JOIN group_master gm ON gm.id = assets_group_id, user_assets_map um WHERE find_in_set( assets_group_id, um.group_id ) AND um.user_id = $user GROUP BY assets_group_id ORDER BY gm.group_name asc";
		}else{
			$sql = "SELECT gm.id, gm.group_name, GROUP_CONCAT( am.id ) as assets FROM assests_master am LEFT JOIN group_master gm ON gm.id = assets_group_id, user_assets_map um WHERE find_in_set( assets_group_id, um.group_id ) GROUP BY assets_group_id ORDER BY gm.group_name asc";
		}
	...
	public function get_areas($user) 
	{
		if(count($user) > 0) {
			...
			$q = "SELECT polyid, polyname FROM `areas` WHERE Audit_Status = 1 AND Audit_Del_Dt is null AND Audit_Enter_uid IN($user_ids) group by polyid ORDER BY polyname ASC";
	...
	public function get_landmarks($user) 
	{
		if(count($user) > 0) {
			...
			$user_ids = implode(',', $user);
			$q = "SELECT id, name FROM landmark WHERE status=1 AND del_date is null AND add_uid IN($user_ids) group by name ORDER BY name ASC ";
			$query = $this->db->query($q);
			return $query->result();
		}
		return false;
	...
	public function get_zones($user) 
	{
		if(count($user) > 0) {
			...
			$user_ids = implode(',', $user);
			$q = "SELECT polyid, polyname FROM `landmark_areas` WHERE Audit_Status = 1 AND Audit_Del_Dt is null AND Audit_Enter_uid IN($user_ids) group by polyid ORDER BY polyname ASC ";
			$query = $this->db->query($q);
			return $query->result();
		}
		return false;
	...
	public function get_subuser($user, $ex_user = '')
	{
		$today=strtolower(date("l"));
		$sql = "select user_id, username, first_name, last_name from tbl_users where status=1 And del_date is null And (find_in_set('$today',display_day) or display_day='all') and admin_id = $user";
		
		if($ex_user != '') $sql .= " and user_id != $ex_user";
	...
	public function get_subuser_detail($uid) 
	{
		...
		$query = $this->db->query("select user_id, username, first_name, last_name, mobile_number, email_address, from_date, to_date, email_alert, sms_alert from tbl_users where user_id=".$uid);
		return $query->result_array();
	...
	public function getAllCoord($user) 
	{
		...
		$qry="SELECT am.id, am.device_id, am.assets_name, lm.lati, lm.longi FROM assests_master am left join tbl_last_point lm on lm.device_id = am.device_id where am.status=1 AND am.del_date is null AND find_in_set(am.id, (SELECT assets_ids FROM user_assets_map where user_id = ".$user." )) ORDER BY am.assets_name";
		//die($qry);
		$query = $this->db->query($qry);
...
s		foreach ($res->result() as $row)
	...
	{
		$device_ids = uri_assoc('id');
		$query = $this->db->query("select am.*, im.icon_path, lp.runtime, lp.data_type, lp.battery_status, lp.alarm_type, ad.division, ao.owner, pm.telecom_provider_name as telecom_provider from assests_master am left join icon_master im on im.id = am.icon_id left join tbl_last_point lp on lp.device_id = am.device_id LEFT JOIN assests_division_master ad ON ad.id = am.assets_division LEFT JOIN assests_owner_master ao ON ao.id = am.assets_owner left join  telecom_provider pm on pm.id=am.telecom_provider where am.id = $device_ids");
		return $query->row();
	}
	public function current_location() 
	{
		...
		$query = $this->db->query("SELECT lp.id, lp.device_id, CONVERT_TZ(lp.add_date,'+00:00','".$this->session->userdata('timezone')."') as add_date,  TIME_TO_SEC(TIMEDIFF( NOW( ), CONVERT_TZ(lp.add_date,'+00:00','".$this->session->userdata('timezone')."'))) as beforeTime, lp.lati, lp.longi, lp.speed, lp.ignition, am.assets_name,am.assets_friendly_nm, am.driver_name, am.driver_mobile,  am.sim_number, lp.address from tbl_last_point lp left join assests_master am on am.device_id = lp.device_id left join icon_master im ON im.id = am.icon_id
	...
	public function distance_today(){
		...
		$query = $this->db->query("select distance from distance_master where add_date BETWEEN '".date('Y-m-d')." 00:00:00' AND '".date('Y-m-d')." 23:59:59' and assets_id = $device_id");
               
		return $query->row();
	}
	
	public function km_reading(){
		$device_id = uri_assoc('id');
		
		$query = $this->db->query("select km_reading from assests_master where id = $device_id");
	...
	public function current_speed(){
		$device_id = uri_assoc('id');
		$sql = "select max(speed) as speed from tbl_track where date(dt) = '".date('Y-m-d')."' and assets_id = '".$device_id."'";
		$query = $this->db->query($sql);
		return $query->row();
	}
		...
		$insert_id = $this->db->insert_id();	
			...
			if($first_landmark == $last_landmark){		//round trip
				...
				$this->db->query($query);
                        ...
                        //Added by Poonam 24-01-2015 13:47 PM
                        $device1=$_POST['route_device'];
			//print_r($device1);
			$i=0;
			...
			for($i=0;$i<$count1;$i++)
			{
			$this->db->query("update assests_master set current_trip = '".$insert_id."' where assests_master.id ='".$device1[$i]."'");
			}
                        //End			
		return $insert_id;
	}
...
//		die($value[0]['photo']);
	...
	public function getOnscreenAlertSettings(){
		...
		$SQL="select onscreen_alert from tbl_users where user_id = $user_id";
		$query = $this->db->query($SQL);
		return $query->row();
	}
	public function groupAssets(){
		...
		if($user_id != 1){
			...
			$SQL = "SELECT am.id, am.assets_name FROM assests_master am, user_assets_map um WHERE find_in_set(am.id, um.assets_ids) AND um.user_id = $user_id AND am.status = 1 AND am.del_date is null";
		}else{
	...
	public function get_subTree(){
		...
		if($group > 0) {
			$sql = "SELECT am.id, am.assets_name FROM assests_master am WHERE id IN (SELECT assets FROM group_master WHERE id = $group)";
		}
		if($area > 0) {
			$sql = "SELECT am.id, am.assets_name FROM assests_master am LEFT JOIN tbl_last_point lp ON lp.device_id = am.device_id LEFT JOIN areas ar ON ar.polyname = lp.current_area WHERE ar.id = $area";
		}
		if($land > 0) {
			$sql = "SELECT am.id, am.assets_name FROM assests_master am LEFT JOIN tbl_last_point lp ON lp.device_id = am.device_id LEFT JOIN landmark ld ON ld.name = lp.current_landmark WHERE ld.id = $land";
		}
		if($owner > 0) {
			$sql = "SELECT am.id, am.assets_name FROM assests_master am WHERE am.assets_owner = $owner";
		}
		if($divs > 0) {
			$sql = "SELECT am.id, am.assets_name FROM assests_master am WHERE am.assets_division = $divs";
		}
	...
	function get_map_data($id){
			...
			$squery="SELECT id, device_id, ignition, address, lati, longi, speed, CONVERT_TZ(add_date,'+00:00','".$this->session->userdata('timezone')."') as add_date FROM ".$this->table_name." WHERE id=$id";
			$query = $this->db->query($squery);
			return $query->result();	
	
	}
	
	public function get_zone_home() 
	{
	...
	public function add_zone() 
	{
		...
		{
			$j = $i + 1;
			$data = array('polyid' => $polyid, 'area_size'=>$area_size, 'color' => $color, 'group_id' => $groups, 'deviceid' => $device, 'polyname' => $nm, 'lat' => round($latAdd[$i],6), 'lng' => round($lngAdd[$i],6), 'pointid' => $j, 'Audit_Enter_Dt' => $datetime, 'Audit_Enter_uid' => $this->session->userdata('user_id'), 'Audit_Status' => 1, 'in_alert'=>$in_alert, 'out_alert'=>$out_alert, 'sms_alert' => $sms_alert, 'email_alert' => $email_alert, 'speed_alert'=>1, 'addressbook_ids'=>$addressbook_ids, 'area_type_opt'=>$area_type_opt);
			
			$query = $this->db->insert_string('landmark_areas', $data);
			//echo $query;
			$this->db->query($query);
	...
	{
		$query = $this->db->query("SELECT * FROM `landmark_areas` lm where polyid = ".$_POST['id'], FALSE);
		return $query->row();
	}
	public function delete_zone() 
	{
		$id = $_POST['id'];
		$res = $this->db->query("update landmark_areas set Audit_Del_Dt = '".gmdate('Y-m-d H:i:s')."', Audit_Status = 0 where polyid=$id");
		return;
	}
	function update_zone() { 
		...
		if($_POST['group'] != '') {
			...
			$rows = $assets_res->result();
			$row = $rows[0];
			$pData['deviceid'] = $row->devices;
		}
		
		$this->db->where('polyid',$_POST['polyid']);
		$this->db->update('landmark_areas',$pData);
		$insert_id = $_POST['polyid'];
	...
	//landmark functions
	public function get_landmark_home() 
	{
		$query = $this->db->query("select area.*, (select group_concat(concat(assets_name,'(',device_id,')')) assets from assests_master where find_in_set(id, area.deviceid)) as assets from landmark_areas area where  Audit_Enter_uid = ".$this->session->userdata('user_id')." and Audit_Status = 1 and Audit_Del_Dt is null");
		return $query->result();
	}

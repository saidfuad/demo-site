...
class Home_model extends Model 
{
	...
	*/
	...
	{
		$query = $this->db->query("select id, group_name from group_master where status=1 AND del_date is null AND add_uid = $user");
		return $query->result();
	...
	public function get_areas($user) 
	{
		if(count($user) > 0) {
			...
			$q = "SELECT polyid, polyname FROM `areas` WHERE Audit_Status = 1 AND Audit_Del_Dt is null AND Audit_Enter_uid IN($user_ids) group by polyid";
	...
	public function get_landmarks($user) 
	{
		if(count($user) > 0) {
			...
			$user_ids = implode(',', $user);
			$q = "SELECT id, name FROM landmark WHERE status=1 AND del_date is null AND add_uid IN($user_ids)";
			$query = $this->db->query($q);
			return $query->result();
		}
		return false;
...
//		die($value[0]['photo']);
	...
	public function groupAssets(){
		...
		$SQL="select id, assets_name from assests_master where find_in_set(id, (SELECT assets_ids FROM user_assets_map where user_id = $user_id))";
		if($group != ""){
			$SQL.=" and find_in_set(id, (select assets from group_master where id = $group))";
		}
	...
	function get_map_data($id){
			...
			$squery="select id, device_id, ignition, address, lati, longi, speed, CONVERT_TZ(add_date,'+00:00','".$this->session->userdata('timezone')."') as add_date from ".$this->table_name." where id=$id Limit 1";

...
function createMarkerMapAll(map, point, html, content, mid){
	var i;
...
function group_dialog_opn(val)
{
	//$("#loading_dialog").dialog("open");
	//$("#loading_top").css("display","block");
        var length = selected_assets_ids.length;
	if(length == 0){
	$("#alert_dialog").html("Please Select Assets");
	$("#alert_dialog").dialog("open");
	}
		...
		function(data) {
			...
			$('.asset_checkbox').each(function(index, object) {
				...
				if (dashboardMarkers[$(this).val()] != undefined) {
					...
					point = dashboardMarkers[$(this).val()].getPosition();
					dashboardBounds.extend(point);
					dMap.setCenter(point);
...
function open_pop_up(header,data,link,type)
{
  ...
  //	topMenuToTab(url, name, id);
	...
	$("#open_pop_up_"+total_pop_up).remove();
	$("#popup").prepend(html);
	setTimeout(function(){ 
      $("#open_pop_up_"+total_pop_up).remove();
    },120000);
	total_pop_up = total_pop_up;
	return "open_pop_up_"+(total_pop_up);
}
...
loadJGrid();
	...
	//$("#main").html('<div id="load" style="padding-top:30%;height: 50%;" align="center"><img src="<?php echo base_url(); ?>assets/style/css/images/ajax.gif" alt="Loading"></div>'+$("#main").html());
	var assetss=selected_assets_ids.split(",");
	$req_obj_xhr = $.post(url, { txt: txt, report:report, paging:grid_paging},
	 function(data) {
		$("#main").html(data);
		//$("#loading_dialog").dialog("close");
		$("#loading_top").css("display","none");
...
//	$("#main input:checked").removeAttr("checked"); // Remove all the checked boxes.
	...
	<?php /* if($this->session->userdata('usertype_id') == 1){ ?>
		...
		$.post("<?php echo base_url(); ?>/index.php/home/changeAssetsCombo/user_id/"+values,
			function(data) {
...
function changePage_img(pg,img_assets_id,time){
	...
	$.post("<?php echo base_url(); ?>/index.php/home/navigationImages/page/"+pg+"/id/"+img_assets_id+"/limit/"+limit+"/time/"+time,
	function(data) {
...
function reloadAssets(){
	...
	if(url == "<?php echo base_url(); ?>index.php/reports/lastpoint") {
		jQuery("#lastpoint_grid").trigger("reloadGrid");
		if(timer_on==1) {
...
function selectedAssets() {
	...
	if(totalSelected.length > 1){
		...
		{
			//var top =$('#main input:checked[value="'+totalSelected[totalSelected.length-1]+'"]').position().top + 15 + $("#tabs").scrollTop();
			var top =$('#map_div').position().top + $("#map_div").height() + 15;
			var left= $('#map_div').width() - 25;
			//var left= $('#main input:checked[value="'+totalSelected[totalSelected.length-1]+'"]').position().left + 15 + $("#tabs").scrollLeft();

			<?php if($this->session->userdata('show_dash_distance_button')==1){ ?>
				// $("#distanceBtn_div").css({top:
				// top ,left: left, position: 'absolute', height:"50px", border:"1px solid lightblue",borderRadius:"4px"});
				$("#distanceBtn_div").css({position: 'absolute', height:"50px", border:"1px solid lightblue", borderRadius:"4px"});
			<?php } ?>
			$('#distanceBtn_div').fadeIn();
		}
	}else{
...
function multiScreen(){
	...
	{
		var selectedAst = selected_assets_ids.split(",");
		$('#tabs').tabs('add', "<?php echo base_url(); ?>index.php/home/multi_map/id/"+selected_assets_ids, 'Multi Screen');
...
function getAssetsDistance(){
	var selectedAst = selected_assets_ids.split(",");
	if(selectedAst.length < 2){
		$("#alert_dialog").html("Select Atleast 2 Assets.");
		$("#alert_dialog").dialog("open");
		return false;
	}
	//$('#distanceBtn_div').hide();
...
function getAssetsDistance_New(from,to){
	$('#tabs').tabs('add', "<?php echo base_url(); ?>index.php/home/map/id/"+from+","+to+"/d/1/cmd/dist", 'Distance');
}
...
//	$(".deviceMain").qtip('hide');
	...
	if (!tabNameExists){
		$('#tabs').tabs('add', "<?php echo base_url(); ?>index.php/home/image_open/id/"+id+"/limit/8", name+" Details");
	}
}
function loadAssetsDash(){
	...
	if(selectedAst.length > 1){
		...
		if (!tabNameExists){
			...
			{
				$('#tabs').tabs('add', "<?php echo base_url(); ?>index.php/home/assets_dash/id/"+selectedAst[0], assetNameArray[selected_assets_ids]+" Details");
			}
...
function chatHeartbeat(){
	...
	$.ajax({
	  url: "<?php echo site_url("home/chat"); ?>?action=chatheartbeat",
	  cache: false,
	  dataType: "json",
...
function closeChatBox(chatboxtitle) {
	...
	$.post("<?php echo site_url("home/chat"); ?>", { action:'closechat',chatbox: chatboxtitle} , function(data){	
	});
...
function checkChatBoxInputKey(event,chatboxtextarea,chatboxtitle) {
	...
	if(event.keyCode == 13 && event.shiftKey == 0)  {
		...
		if (message != '') {
			...
			$("#chatbox_"+chatboxtitle+" .chatboxcontent").scrollTop($("#chatbox_"+chatboxtitle+" .chatboxcontent")[0].scrollHeight);
			$.post("<?php echo site_url("home/chat"); ?>", {action:'sendChat',to: chatboxtitle, message: message} , function(data){
			});
		}
		chatHeartbeatTime = minChatHeartbeat;
		chatHeartbeatCount = 1;
...
function startChatSession(){  
	$.ajax({
	  url: "<?php echo site_url("home/chat"); ?>?action=startchatsession",
	  cache: false,
	  dataType: "json",
	  success: function(data) {
...
function filterAssetsCombo(val, cid){
	$.post("<?php echo base_url(); ?>index.php/home/filter_assets", { grp: val},
	 function(data) {
		$("#"+cid).dropdownchecklist('destroy');
		$("#"+cid).html("<option value=''><?php echo $this->lang->line("Select All"); ?></option>"+data);
...
function view_map(id,asset){
	var nameToCheck = asset;
	var tabNameExists = false;
	...
	$('#tabs ul.ui-tabs-nav li a').each(function(i) {
		if (this.text == nameToCheck) {
			...
			$('#tabs').tabs('add', '<?php echo base_url(); ?>index.php/home/view_map/id/'+id+'/asset/'+asset, nameToCheck);
			return false;
		}
	});
	if (!tabNameExists){
...
function dashboard_view_map(id,ast){
	var tabNameExists = false;
	...
	$('#tabs ul.ui-tabs-nav li a').each(function(i) {
		if (this.text == ast) {
			tabNameExists = true;
			$('#tabs').tabs('remove', $(this).attr("href"));
			$('#tabs').tabs('add', '<?php echo base_url(); ?>index.php/home/view_map/id/'+id+'/asset/'+ast, ast);
			return false;
		}
	});
	if (!tabNameExists){
 ...
 function changeImage(element) {
     element.src = element.bln ? "<?php echo base_url(); ?>assets/dashboard/images/blue_mute.png" : "<?php echo base_url(); ?>assets/dashboard/images/blue_sound.png";
 ...
 var thissound1=document.getElementById("sound_start");
function enableMute()
  { 
   //$("#a1").attr('src','mute.gif');
   
  thissound1.muted=true;
  } 
function disableMute()
  { 
      // $("#a2").attr('src','mute.gif');
 
  thissound1.muted=false;
  } 

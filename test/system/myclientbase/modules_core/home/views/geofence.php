...
<?php
$dCombonm="cmbAGroup_".time();
$dCombo = "<select class='select ui-widget-content ui-corner-all' id='cmbAGroup_".time()."' style='height:100px;' name='cmbDevice' multiple='multiple'>";
$dCombo .= $deviceGrp;
$dCombo .= "</select>";
$dCombo_land = "<select id='area_device_".time()."' style='height:100px;' name='land_device' multiple='multiple'>";
...
<?php
  ...
  control_UI.title = '<?php echo $this->lang->line("Click to Create GeoFence"); ?>';
  ...
  areaText.innerHTML = '<?php echo $this->lang->line("Area"); ?>';
...
<?php
  ...
  google.maps.event.addDomListener(control_UI, 'click', function() {
	if($("#search_geo").css('display') == 'block'){
		...
		if(placesAreaMarker != null) placesAreaMarker.setMap(null);
	}else{
   ...
   google.maps.event.addDomListener(control_UI, 'click', function() {
	if($("#search_geo1").css('display') == 'block'){
		$("#search_geo1").css('display', 'none');
		if(placesAreaMarker != null) placesAreaMarker.setMap(null);
	}else{
		$("#search_geo1").css('display', 'block');
	}	
  });
   <?php } ?>
}
...
var htmltext = '<table class="formtable" id="poly_main<?php echo time(); ?>" style="display:none">';
	...
	htmltext +=	'</tr>';
	htmltext +=	'<tr>';
	htmltext +=	'<td id="search_geo1"><?php echo $this->lang->line("Search"); ?></td>';
	//htmltext +=	'<td><input type="text" class="text ui-widget-content ui-corner-all" style="width:100%" class="not_err" name="search_c" id="search_c_1<?php echo time(); ?>"  /></td>';
	htmltext +=	'<td ><input id="search_c_1" style="width:100%;" class="text ui-widget-content ui-corner-all" type="text"/></td>'
	htmltext +=	'</tr>';
	htmltext +=	'<tr style="line-height:10px">';
	htmltext +=	'<td>&nbsp;</td>';
	htmltext +=	'<td><span><a href="#" style="float:right;margin-right:15px;color:blue;font-size:10px" onclick="selectAllarea<?php echo time(); ?>(\'<?php echo "#cmbAGroup_".time(); ?>\')"><?php echo $this->lang->line("Select/Unselect All"); ?></a></span>';
	htmltext += '</td>';
	htmltext +=	'</tr>';
	htmltext +=	'<tr>';
	htmltext +=	'<td style="vertical-align: middle !important;"><?php echo $this->lang->line("Assets"); ?></td>';
	htmltext +=	'<td>';
...
*/	
	...
	htmltext +=	'<tr>';
	htmltext +=	'<td style="font-size:14px;"><input type="button" onclick="creatNew()" value="<?php echo $this->lang->line("Create New"); ?>" style="margin-top:3px"></td>';		
	htmltext +=	'</tr>';
	htmltext +=	'<tr>';
	...
	htmltext +=	'</tr>';
	htmltext +=	'<tr>';
	htmltext +=	'<td style="font-size:14px;"><input type="button" onclick="addPointfrm_Combo()" value="<?php echo $this->lang->line("Set Selected"); ?>" style="margin-top:3px"></td>';		
	htmltext +=	'</tr>';
	htmltext +=	'</table>';
...
function creatNew(){
	...
	newPoly = false;
	clearMarker();
	selectAllarea_select<?php echo time(); ?>("#cmbAGroup_<?php echo time(); ?>");
}
...
function editArea(id,edit_poly){
	...
	$.post("<?php echo base_url(); ?>index.php/home/edit_area", { id: id},
	 function(result) {
	 	$("#dialog_poly<?php echo time(); ?>").html(htmltext);
		$("#poly_main<?php echo time(); ?>").show();
		$("#poly_help<?php echo time(); ?>").hide();
		...
		if(grps != null && grps != '') {
			grps = grps.split(",");
			$("#cmbAGroup_<?php echo time(); ?>").val(grps);
		}
/*		
...
function confirmDeleteArea(){
	...
	$.post("<?php echo base_url(); ?>index.php/live/deletepoly", { id: polyid },
	 function(data) {
		$("#alert_dialog").html("<?php echo $this->lang->line("Area Deleted Successfully"); ?>");
...
function addPointfrm_Combo(){
	...
	if(lt1!=0 && lng1!=0){		
		...
		alert("<?php echo $this->lang->line("Location not Found"); ?>");
	}
...
function addPoly<?php echo time(); ?>(){
	...
	if(polyname == "" || polyname == null){
		alert("<?php echo $this->lang->line("Please enter area name"); ?>");
		document.getElementById('txtPoly1<?php echo time(); ?>').focus();
		return false;
	}
	
	if(polyMarkers.length < 3){
		alert("<?php echo $this->lang->line("Please select area on map"); ?>");
		return false;
	}
	
	grpId = $("#cmbAGroup_<?php echo time(); ?>").val();
	if(grpId)
		grpId = grpId.join(",")
	else
		grpId = '';
...
*/		
	...
	$.post(sUrl,
		{in_alert:in_alert, out_alert:out_alert, sms_alert:sms_alert, email_alert:email_alert, group:grpId, name:polyname, latAdd:polyLat, lngAdd:polyLng, addressbook_ids:addressbook_ids, area_size:area_size, color:$('#color<?php echo time(); ?>').val(), area_type_opt:area_type_opt},
		function(data){
...
function updatePoly<?php echo time(); ?>(){
	...
	if(devId)
		...
		devId = '';
	*/		
	var sUrl = "<?php echo base_url(); ?>index.php/home/updateArea";
...
function createPlacesAreaMarker(point, map) {
...
$(document).ready(function () {
	...
	map_geo.setZoom(20);
	placesAreaMarker = createPlacesAreaMarker(location, map_geo);
      }
    });
  });
   ...
   $(function() {
	 addPolyNew();
    $("#poly_main<?php echo time(); ?> #search_c_1").autocomplete({
      //This bit uses the geocoder to fetch address values
      source: function(request, response) {
        geocoder.geocode( {'address': request.term }, function(results, status) {
          response($.map(results, function(item) {

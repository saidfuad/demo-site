...
<?php
$dCombo = "<select id='cmbLGroup_".time()."' style='height:65px;' name='cmbGroup' multiple='multiple'>";
$dCombo .= $deviceGrp;
$dCombo .= "</select>";
...
<?php
  ...
  google.maps.event.addDomListener(control_UI, 'click', function() {
	if($("#search_landmark").css('display') == 'block'){
		...
		if(placesMarker != null) placesMarker.setMap(null);
	}else{
...
var htmltext_landmark = '<table class="formtable" id="land_main<?php echo time(); ?>" style="display:none;">';
	...
	htmltext_landmark +=	'<td colspan="2" style="font-size:14px;"><?php echo $this->lang->line("Click_on_Map_For_Location"); ?></td>';		
	htmltext_landmark +=	'</tr>';
	htmltext_landmark +=	'<tr>';
	...
	htmltext_landmark +=	'<input type="text" class="text ui-widget-content ui-corner-all" id="land_name_<?php echo time(); ?>" onmouseover="this.focus();"><br/></td>';
	htmltext_landmark +=	'<td><?php echo $this->lang->line("Address"); ?>';
	htmltext_landmark +=	'<br/><input type="text" class="text ui-widget-content ui-corner-all" id="land_address_<?php echo time(); ?>" >';
	htmltext_landmark += 	'</td>';
	htmltext_landmark +=	'</tr>';	
	htmltext_landmark +=	'<tr>';
	htmltext_landmark +=	'<td><?php echo $this->lang->line("Radius"); ?>';
	htmltext_landmark +=	'<br/><div style="width:50%;float:left;"><input type="text" class="text ui-widget-content ui-corner-all" id="land_radius_<?php echo time(); ?>" value="2" style="width:55px;" /></div><div style="width:50%;float:right;"><select class="select ui-widget-content ui-corner-all" id="distance_unit_<?php echo time(); ?>"><option>KM</option><option>Mile</option><option>Meter</option></select></div><br/><?php echo $this->lang->line("Icon"); ?><br/><?php echo $icon_combo; ?></td>';
	htmltext_landmark +=	'<td style="vertical-align: middle !important;"><span><?php echo $this->lang->line("Assets_List"); ?>:<span><a href="#" style="float:right;margin-right:15px;color:blue;font-size:10px" onclick="selectAlllandmark<?php echo time(); ?>(\'<?php echo "#land_device_".time(); ?>\')"><?php echo $this->lang->line("Select/Unselect All"); ?></a></span> </span>';
...
*/	
	...
	htmltext_landmark +=	'<tr>';
	htmltext_landmark +=	'<td colspan="2"><?php echo $this->lang->line("Alert Before Landmark"); ?>&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" name="alert_before_landmark" id="alert_before_landmark<?php echo time(); ?>" class="text ui-widget-content ui-corner-all" value="0" style="width:50px;">&nbsp;KM';
	htmltext_landmark +=	'</tr>';
	htmltext_landmark +=	'<tr>';
	
	htmltext_landmark +=	'<tr>';
	htmltext_landmark +=	'<td colspan="2"><?php echo $this->lang->line("In Alert"); ?>&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" name="in_alert" id="landmark_in_alert<?php echo time(); ?>" checked="true">&nbsp;&nbsp;&nbsp;<?php echo $this->lang->line("Out Alert"); ?>&nbsp;&nbsp;&nbsp;<input type="checkbox" name="out_alert" id="landmark_out_alert<?php echo time(); ?>" checked="true"></td>';
	htmltext_landmark +=	'</tr>';
	...
	*/
	htmltext_landmark +=	'<tr>';
	htmltext_landmark +=	'<td style="font-size:14px;"><input type="button" onclick="creatNew()" value="<?php echo $this->lang->line("Create New"); ?>" style="margin-top:3px"></td>';		
	htmltext_landmark +=	'</tr>';
	htmltext_landmark +=	'<tr>';
	htmltext_landmark +=	'<tr>';
	htmltext_landmark +=	'<td style="font-size:14px;"><h4><?php echo $this->lang->line("OR"); ?> </h4></td>';
	htmltext_landmark +=	'</tr>';
	htmltext_landmark +=	'<tr>';
	htmltext_landmark +=	"<td style='font-size:14px;'><?php echo $this->lang->line("Vehicle Current Position"); ?> : <br><select id='live_assets_cmb<?php echo time(); ?>' style='margin-top: 5px;padding: 0.4em;width: 94%;' class='select ui-widget-content ui-corner-all'><?php echo $live_combo; ?></select></td>";		
	htmltext_landmark +=	'</tr>';
	htmltext_landmark +=	'<tr>';
	htmltext_landmark +=	'<td style="font-size:14px;"><input type="button" onclick="getSelectedCoords()" value="<?php echo $this->lang->line("Set Landmark"); ?>" style="margin-top:3px"></td>';		
	htmltext_landmark +=	'</tr>';
	/*
	htmltext_landmark +=	'<tr>';
...
function createLanbmark<?php echo time(); ?>(){
	...
	var in_alert = $("#landmark_in_alert<?php echo time(); ?>").is(':checked');
	var out_alert = $("#landmark_out_alert<?php echo time(); ?>").is(':checked');
	//landLng, landLng	
	...
	var assets_list = $("#land_device_<?php echo time(); ?>").val();
	if(assets_list == "" || assets_list == null){
		$("#alert_dialog").html('<?php echo $this->lang->line("Please_Select_Assets_For_Landmark"); ?>');
		$("#alert_dialog").dialog('open');
		$("#land_name_<?php echo time(); ?>").focus();
		return false;
	}
	...
	grpId = $("#land_device_<?php echo time(); ?>").val();
	if(grpId){
			grpId = grpId.join(",");
	}else{
		grpId = '';
	...
	$.post(sUrl,
		{id:land_id, icon:icon, group:grpId, name:land_name, address:land_address, radius:land_radius, distance_unit:distance_unit, lat:landLat, lng:landLng,group_nm :landmark_group_naget, sms_alert:sms_alert, email_alert:email_alert, addressbook_ids:addressbook_ids,comments:comments,alert_before_landmark:alert_before_landmark, in_alert:in_alert, out_alert:out_alert},
		function(data){
...
function editLandmark(id,point){
	...
	$.post("<?php echo base_url(); ?>index.php/home/edit_landmark", { id: id },
	 function(result) {
		...
		$("#landmark_group_nm_<?php echo time(); ?>").val(result.data.device_ids);
		var grps = result.data.device_ids;
		if(grps != null && grps != '') {
			grps = grps.split(",");
			$("#land_device_<?php echo time(); ?>").val(grps);
		}
		/*
...
function createPlacesMarker(point, map) {
	...
	$(function() {
		...
		$("#address_landmark").autocomplete({
		  ...
		  select: function(event, ui) {
			...
			map_landmark.setZoom(15);
			placesMarker = createPlacesMarker(location, map_landmark);
			placesMarker.setMap(null);
		  }
		});
	  });

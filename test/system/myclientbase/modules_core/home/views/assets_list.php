...
function paginate($reload, $page, $tpages, $adjacents, $totalRecords, $limit, $language_formate) {
	...
	$out = '<div class="sixteen columns centre" id="bottomPaging" style="display:none">';
	if($tpages>1 && $page!=1)
	{
	...
	if($limit == 100)
		$out.= " selected='selected'";
	$out.= ">100</option><option";
	if($limit == 200)
		$out.= " selected='selected'";
	$out.= ">200</option><option";
	if($limit == 300)
		$out.= " selected='selected'";
	$out.= ">300</option><option";
	if($limit == 400)
		$out.= " selected='selected'";
	$out.= ">400</option><option value='10000'";
	if($limit == 500)
		$out.= " selected='selected'";
	$out.= ">500</option><option value='10000'";
	if($limit == 600)
		$out.= " selected='selected'";
	$out.= ">600</option><option value='10000'";
	if($limit == 700)
		$out.= " selected='selected'";
	$out.= ">700</option><option value='10000'";
	if($limit == 800)
		$out.= " selected='selected'";
	$out.= ">800</option><option value='10000'";
	if($limit == 900)
		$out.= " selected='selected'";
	$out.= ">900</option><option value='10000'";
	if($limit == '10000')
		$out.= " selected='selected'";
	$out.= ">All</option>";
	$out.= "</select></span></div>";
...
<style>
	...
	<?php if(count($coords) > 0) { 
		foreach ($coords as $coord) {
			if($coord->assets_category_id!=""){
                                            
                          	$image_type=$coord->image_type;              
                    	}
                    	else{
                    	    $image_type="car.png";
                    	}
			...
			if($coord->ignition == 0)
				...
				$ignition = "ON";
			$text .= $this->lang->line("Ignition:").$ignition." ," .$this->lang->line("Speed:").$coord->speed." KM<br>";
			...
			//if($this->session->userdata('show_dash_legends')==1){
				$text .= $this->lang->line("Status: ");
				if($seconds_before < $this->session->userdata('network_timeout') && $coord->speed > 10 && $coord->ignition == 1 && $seconds_before != ""){
						$status ="Running";
						$status_img = "green_dot.png";
						$color = "green";
				}else if($seconds_before < $this->session->userdata('network_timeout') && $coord->ignition == 0 && $seconds_before != ""){
						$status ="Parked";
			...
			if($status == "Parked")
				$text .=$this->lang->line("Parked From: ").$coord->stop_from."<br>";
/*			
			if($coord->routename != ""){
...
*/			
			...
			if($this->session->userdata('show_map_driver_detail_window')==1){
				...
				$text .=$this->lang->line("Driver Name: ").$coord->driver_name."<br>"; 
			
				if($coord->driver_mobile!="" || $coord->driver_mobile!=null) 
				$text .=$this->lang->line("Driver Mob.:").$coord->driver_mobile."<br>"; 
			}
	...
	?>
		...
		<?php if((floatval($coord->lati) != 0) && (floatval($coord->longi) != 0)) { ?>
			...
			var boxText = "<div style='line-height: 15px; font-size: 11px; font-weight: bold; color: #2E6E9E; border: 1px solid black; background: none repeat scroll 0% 0% #DFEFFC; padding: 4px; margin: 0px 0px 0px 45px; text-align:center; -moz-border-radius: 8px; border-radius: 8px; white-space: nowrap;'><img src='<?php echo base_url(); ?>assets/images/<?php echo $status_img; ?>' title='<?php echo $status; ?>'><img src='<?php echo base_url(); ?>assets/images/direction.jpg' style='transform: rotate(<?php echo intval($direction);?>deg);-ms-transform:rotate(<?php echo intval($direction);?>deg);-webkit-transform:rotate(<?php echo intval($direction);?>deg);' title='Direction'>&nbsp;<?php echo $coord->assets_name ?></div>";
			...
			dLabel.open(dMap);
			dLabelArr[<?php echo $coord->assets_id ?>] = dLabel;
		
		dashboardMarkers[<?php echo $coord->assets_id ?>] = createMarker(dMap, point, '<?php echo $coord->assets_name; ?>', "<div style='text-align:left;'><?php echo $text; ?></div>", '<?php echo $image_type; ?>', '', 'sidebar_map', '', '', <?php echo $coord->assets_id; ?>);
	...
	//	if($coord->speed >= $coord->old_speed ){ $disp_icon="SpeedUp.png"; }else{ $disp_icon="SpeedDown.png";}
		...
		<li style="display:inline-block;white-space:nowrap;float:none;"><div class="deviceMain <?php echo $class; ?>">
			...
			<div
			style="float:right;width:100px;padding-top:7px;font-size:14px;font-weight:bold;"><a
			style="cursor:pointer;" onclick="directTab('<?php echo $coord->device_id; ?>', <?php echo $coord->assets_id; ?>)" value="<?php echo date("$date_format $time_format" ,strtotime($coord->add_date)); ?>(<?php echo $coord->received_time; ?>)"><?php echo $coord->assets_name; ?></a></div>
			...
			<div style="line-height:15px" class="ui-widget-content">
				<div style="font-weight:bold;color:#000;font-size:17px;">
					<span class="first_t_link" onmouseover="displayFirst()" style="cursor:pointer;color:#2E6E9E;font-size:11px;text-decoration:underline;"><?php echo $this->lang->line('Vehicle'); ?></span>
					&nbsp;&nbsp;
					
					<span class="second_t_link" onmouseover="displaySecond()" style="cursor:pointer;color:#2E6E9E;font-size:11px;"><?php echo $this->lang->line('Data'); ?></span>
				...
				<span style='display:block'>
				<?php echo $this->lang->line('Parked From') ?> : <?php echo $coord->stop_from; ?> 
				</span><?php } ?>
				...
				<span style='display:block'>
				<?php echo $this->lang->line('Ignition Status') ?> : <?php echo ($coord->ignition == 1 ? "ON" : "OFF"); ?> 
				</span><?php } ?>
				...
				<span><input type="button" value="<?php echo $this->lang->line('View Dashboard'); ?>" class="ui-button ui-widget ui-state-default ui-button-text-only" onClick="loadAssetsDash_tt(<?php echo $coord->assets_id; ?>,'<?php echo $coord->assets_name; ?>')"/></span>
				<?php } ?>
				</div>
				<div class="second_t" style="display:none;">
					<?php if($coord->reason_text!=""){ ?>
					<span style='display:block'>
					<?php echo "<b> ".$this->lang->line('Status')." :</b> ".$coord->reason_text; ?> 
					</span><?php } ?>
					...
					<?php if($coord->device_id!=""){ ?>
					<span style='display:block'>
					<?php echo "<b>IMEI Number :</b> ".$coord->device_id; ?>
					</span><?php } ?>

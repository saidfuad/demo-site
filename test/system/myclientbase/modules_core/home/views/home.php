...
<link rel="shortcut icon" href="<?php echo base_url(); ?>assets/images_login/logo_itms.png">
<?php /*
<!-- Atul --->
...
var assets_combo_opt="<option value=''><?php echo $this->lang->line("Please Select"); ?></option><?php echo $usr_assets_cmb; ?>";
<?php }else{ ?>
var assets_combo_opt="<?php echo $usr_assets_cmb; ?>";
<?php } ?>
<?php if($usr_assets_cmb_count>1){ ?>
var assets_combo_opt_report="<option value=''><?php echo $this->lang->line("Select All"); ?></option><?php echo $usr_assets_cmb; ?>";
<?php }else{ ?>
...
var opts_dmap = null;
var nearestDisplay;
var nearestService = new google.maps.DirectionsService();
	...
	$.post("<?php echo base_url(); ?>index.php/home/extra_js",
			 function(data) {	
				...
				dashboardMap = document.getElementById("map_div");
					...
					nearestDisplay = new google.maps.DirectionsRenderer({suppressMarkers: true});
...
function loadVehicleDistanceValue(){
		...
		htm += "</table><br><a style='cursor:pointer' onclick='loadVehicleDistanceData()'><?php echo $this->lang->line("Refresh"); ?></a>&nbsp;&nbsp;<a style='cursor:pointer' onclick='saveVehicleDistanceData()'><?php echo $this->lang->line("Save"); ?></a>";
		$("#dispMap").append(htm);
		jQuery("input:button, input:submit, input:reset").button();
	}
...
function setMenuXY(caurrentLatLng){
	...
	var menuWidth = $('.nearestmenu').width();
	var menuHeight = $('.nearestmenu').height();
	...
	$('.nearestmenu').css('left',x);
	$('.nearestmenu').css('top',y );
};
...
function showContextMenu(event) {
	...
	$('.nearestmenu').remove();
	  ...
	  contextmenuDir.className = 'nearestmenu';
	  contextmenuDir.innerHTML = "<a id='show_nearest' onClick='find_closest_marker("+event.latLng.lat()+","+event.latLng.lng()+");'><div class=context>Show Nearest Asset<\/div><\/a><a id='clear_nearest' onClick='clearNearest();'><div class=context>Clear Map<\/div><\/a>";
...
//    var lng = event.latLng.lng();
	...
	var assetSelected = selected_assets_ids.split(",");
	$.each(assetSelected, function( index, value ) {
	if( dashboardMarkers[value] !== undefined ) {
            var mlat = dashboardMarkers[value].position.lat();
            var mlng = dashboardMarkers[value].position.lng();
	...
	placeMarker( lat, lng, dashboardMarkers[closest] );
//    alert(dashboardMarkers[closest].title);
}

function placeMarker(lat, lng, nrt) {
...
//    nearestMarker.setMap(null);
    ...
    nearestMarker = new google.maps.Marker({
        ...
        map: dMap,
		title: "Nearest Asset : " + nrt.title
    });
	google.maps.event.addListener(nearestMarker, 'click', function() {
    	nearestInfo.open(dMap,nearestMarker);
  	});

	nearestDisplay.setMap(dMap);
	calcNearest(lat, lng, nrt);
	$('.nearestmenu').remove();
}
...
function clearNearest() {
	nearestMarker.setMap(null);
	$('.nearestmenu').remove();
	nearestDisplay.setMap(null);
}

function calcNearest(lat, lng, nrtMarker) {
  var nearestInfoString = '';
  var start = new google.maps.LatLng(lat, lng); 
  var end = new google.maps.LatLng(nrtMarker.position.lat(), nrtMarker.position.lng());
  ...
  nearestService.route(request, function(response, status) {
    if (status == google.maps.DirectionsStatus.OK) {
      nearestDisplay.setDirections(response);
		nearestInfoString += 'Nearest Asset: ' + nrtMarker.title + "<br/>";
		nearestInfoString += 'Distance KM: ' + response.routes[0].legs[0].distance.text + "<br/>";
		nearestInfoString += 'Time Required to Reach: ' + response.routes[0].legs[0].duration.text + "<br/>";
		
		nearestInfo = new google.maps.InfoWindow({
		  content: nearestInfoString
		});
    }
  });
}
    ...
    <div class="ui-layout-north" style="height:120px;">
		...
		<div style="top: 0px; font-weight:bold;float:right;">
			...
			<div style="height:30px;display:block;float:right; margin-bottom:5px">
				...
				<span style='display:block;text-align:left;'><img src="<?php echo base_url(); ?>assets/images/blue.png"/> Server Load is between 51% to 80% </span>
				...
				<a href="#" style="float:left;padding:2px" ><img src="<?php echo base_url(); ?>assets/dashboard/images/blue_sound.png"onclick="changeImage(this)" /></a>
		...
		<div class="sixteen columns" id="header" style="float:left;width:80%;padding-bottom:0px;display:none;">
		...
		<!-- By:Prashant D. Reasion: to change the header appearance -->
		<div class="five columns" style="float:left;margin:0 20px;">
		<img style= "margin:20px 30px;" src='<?php echo base_url(); ?>assets/images_login/logo_itms.png'></img>
		...
		<?php 
				if($this->session->userdata('usertype_id')==1){
					...
					$SQL = "select concat(group_concat(mus.menu_id),',',mm.parent_menu_id) as data from  mst_user_profile_setting mus left join main_menu_master mm on mm.id=mus.menu_id where mus.profile_id = '".$this->session->userdata('profile_id')."' ";
					// and mus.setting_name='main
					...
					for($i=0;$i<count($menu_id);$i++)
					{
						if($parent_menu_id[$i]==$parent_id)
						{
							...
							if($where_to_show[$i] == 'link'  and $parent_id == ""){
								...
								} else if('itmsafrica.com' == $_SERVER['HTTP_HOST'] || 'www.itmsafrica.com' == $_SERVER['HTTP_HOST']){
									echo " onclick='' ";
                                  }
	...
	<div class="ui-layout-center" id="tabs" style="float:left;position:relative;">
			<ul>
				...
				<li class="addons" id="testAddons">
					<img src="<?php echo base_url(); ?>assets/images/map.png" alt="Show/Hide Map" title="<?php echo $this->lang->line('Show/Hide Map'); ?>" onClick="show_hide_tree_map()" style="cursor:pointer;float:left;" height="32" width="32" />
					<img id="imgmaxmin" src="<?php echo base_url(); ?>assets/style/img/icons/window_full_screen.png" style="cursor:pointer;float:left" alt="max" title="<?php echo $this->lang->line("Maximize"); ?>" onClick="maximize(this)" />
					...
					<div class="twelve columns alpha">
                        ...
                        <div style="width:70%;float:left; font-size: 1em; font-family: Lucida Grande,Lucida Sans,Arial,sans-serif;">
							<table width="100%" cellspacing="3" cellpadding="5" style="margin: 0px 5px;">
								<tr>
								<td width="15%"><?php echo $this->lang->line('Users'); ?><br><select class="optdetail" id="opt_users" style="border: 1px solid #CCCCCC; border-radius: 2px; color:#444444; padding:6px 4px;" onChange="changeReport(); setcombo('opt_users', this.value);"><option value=''><?php echo $this->lang->line('All Users'); ?></option><?php echo $subUserOpt; ?></select></td>
								<td width="15%"><?php echo $this->lang->line('Groups'); ?><br><select class="optdetail" id="opt_groups" style="border: 1px solid #CCCCCC; border-radius: 2px; color:#444444; padding:6px 4px;" onChange="changeReport(); setcombo('opt_groups', this.value);"><option value=''><?php echo $this->lang->line('All Groups'); ?></option><?php echo $groupOpt; ?></select></td>
								<td width="15%"><?php echo $this->lang->line('Areas'); ?><br/><select class="optdetail" id="opt_areas" style="border: 1px solid #CCCCCC; border-radius: 2px; color:#444444; padding:6px 4px;" onChange="changeReport(); setcombo('opt_areas', this.value);"><option value=''><?php echo $this->lang->line('All Areas'); ?></option><?php echo $areasOpt; ?></select></td>
								<td width="15%"><?php echo $this->lang->line('Landmarks'); ?><br/><select class="optdetail" id="opt_landmarks" style="border: 1px solid #CCCCCC; border-radius: 2px; color:#444444; padding:6px 4px;" onChange="changeReport(); setcombo('opt_landmarks', this.value);"><option value=''><?php echo $this->lang->line('All Landmarks'); ?></option><?php echo $landOpt; ?></select></td>
<?php if($this->session->userdata('show_owners') == 1){ ?>
				                <td width="15%"><?php echo $this->lang->line('Owners'); ?><br/><select class="optdetail" id="opt_owners" style="border: 1px solid #CCCCCC; border-radius: 2px; color:#444444; padding:6px 4px;" onChange="changeReport(); setcombo('opt_owners', this.value);"><option value=''><?php echo $this->lang->line('All Owners'); ?></option><?php echo $ownerOpt; ?></select></td>
<?php } ?>
<?php if($this->session->userdata('show_divisions') == 1){ ?>
				                <td width="15%"><?php echo $this->lang->line('Divisions'); ?><br/><select class="optdetail" id="opt_divisions" style="border: 1px solid #CCCCCC; border-radius: 2px; color:#444444; padding:6px 4px;" onChange="changeReport(); setcombo('opt_divisions', this.value);"><option value=''><?php echo $this->lang->line('All Divisions'); ?></option><?php echo $divisionOpt; ?></select></td>
<?php } ?>
                                <td width="15%">Zones<br/><select class="optdetail" id="opt_zones" style="border: 1px solid #CCCCCC; border-radius: 2px; color:#444444; padding:6px 4px;" onChange="changeReport(); setcombo('opt_zones', this.value);"><option value=''><?php echo $this->lang->line('All Zones'); ?></option><?php echo $zonesOpt; ?></select></td>
								</tr>
							</table>
                        </div>
		                <?php } ?>
			...
			<!--a onClick="group_dialog_opn(1);" title="<?php echo $this->lang->line('add_to_group'); ?>"><img src="<?php echo base_url(); ?>assets/images/truck-group.jpg" height="32" width="32" /></a -->
                        ...
                        <?php } ?>     
						<img src="<?php echo base_url(); ?>assets/images/distance-icon.png" alt="Distance" title="<?php echo $this->lang->line('Distance'); ?>" onClick="getAssetsDistance()" style="cursor:pointer;height: 35px; width: 35px;" id="select_assest_dist_view"/>
	...
	<center>&copy; <?php echo date("Y"); ?> <?php echo $this->lang->line('itms_footer'); ?><a href="" target="_blank"><?php echo $this->lang->line('itms1'); ?></a><br></center>
	</div>

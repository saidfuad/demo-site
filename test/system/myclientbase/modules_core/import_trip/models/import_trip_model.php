<?php 
class Import_trip_model extends Model 
{
	/**
	* Instanciar o CI
	*/
	...
	function getAllData(){
		...
		$SQL = "SELECT count(*) as total FROM tbl_routes as tr left join assests_master as am on find_in_set(am.current_trip,tr.id) WHERE tr.status=1 and tr.del_date is null and tr.add_uid = ".$this->session->userdata('user_id');
		...
		$SQL = "SELECT tr.id ,tr.routename, GROUP_CONCAT(am.assets_name) as assets FROM  tbl_routes as tr left join assests_master as am on find_in_set(am.current_trip,tr.id) WHERE tr.status=1 and tr.del_date is null and tr.add_uid = ".$this->session->userdata('user_id');
	...
	function getIds($id)
	{
		$SQL = "SELECT device_id, assets_name from assests_master as am left join tbl_routes as tr on FIND_IN_SET(am.device_id,tr.deviceid) WHERE tr.id=".$id;
		$query = $this->db->query($SQL);

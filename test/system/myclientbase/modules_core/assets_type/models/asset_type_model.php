...
class Asset_type_model extends Model 
{
	...
	* Instanciar o CI
	*/
	public function Asset_type_model()
    {
        parent::Model();
	...
	function getAllData(){
		...
		$SQL = "SELECT * FROM assests_type_master where status=1 and del_date is null";
		if($where != "")
			$SQL .= " AND $where";
	...
	public function delete_assets() 
	{
		...
		$delete_assets = $this->db->query("UPDATE `assests_type_master` SET status=0, del_uid=".$this->session->userdata('user_id').", del_date='".$date."' WHERE id in(".$ids.")");
	...
	function save($db_array, $id=NULL, $set_flashdata = TRUE) {
		...
		$user = $this->session->userdata('user_id');
		$id = uri_assoc('id');
		$this->db->insert("assests_type_master", $db_array);
		return $success;
	...
	function check_assets_type()
	{
		$name = uri_assoc('nm');
		$id = uri_assoc('id')?uri_assoc('id'):-1;
	
		$this->db->where("assets_type_nm",$name);
		$this->db->where_not_in("id",$id);
		$this->db->where("status",1);
	//	$this->db->where("add_uid",$this->session->userdata('id'));
		//$this->db->where_not_in('id', $id);
		$this->db->from('assests_type_master');

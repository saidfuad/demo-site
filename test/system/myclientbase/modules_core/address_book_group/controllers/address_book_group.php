...
class Address_book_group extends Admin_Controller {
	
	function __construct() {

		parent::__construct(TRUE);

		$this->load->model('address_book_group_model','',TRUE);
		$this->load->model('form_model','',TRUE);
	}
	function index()
	{
		$this->load->view('address_book_group_grid');
	}
	function loadData(){
		
		$data = $this->address_book_group_model->getAllData(); 
	...
	function deleteData(){
		//die("found");
		//echo $this->address_book_group_model->delete_assets(); 
		$this->output->set_output($this->address_book_group_model->delete_assets());
	}
...
//		print_r($data);
	...
	function form() {
		...
		if (!$this->form_model->validate()) {
			...
			/*$rows = $this->address_book_group_model->prepare_icon();
			$iconOpt = '';
			foreach ($rows as $row) {
			...
			if(uri_assoc('id')){
				...
				$this->address_book_group_model->save($formdata, uri_assoc('id'));
			}
	...
	function asset()
	{
		$this->load->model('address_book_group_model');
		$rows = $this->address_book_group_model->get_locations();
		foreach ($rows as $row) {
            $data['lat'] = $row->lati;
			$data['lng'] = $row->longi;
			$data["last_id"] = $row->id;
	...
	function chk_nm()
	{
		$rows = $this->address_book_group_model->check_Addess_Group();
		//die($rows);
		//echo $rows;

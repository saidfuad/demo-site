...
jQuery().ready(function (){
	...
	$("#area_in_out_pager option[value=10000]").text('All');
	$("#area_in_out_pager .ui-pg-selbox").change(function(){
		grid_paging=$("#area_in_out_pager .ui-pg-selbox").val();
	});
	...
	jQuery("#area_in_out_grid").jqGrid("navButtonAdd","#area_in_out_pager",{caption:"<?php echo $this->lang->line("Export"); ?>",
		onClickButton:function(){
			...
			var myPostData = $('#area_in_out_grid').jqGrid("getGridParam", "postData");
			var sidx = myPostData.sidx;
			var sord = myPostData.sord;
			// sdate="+sdate+"&edate="+edate+"&
			var qrystr ="/export?sdate="+sdate+"&edate="+edate+"&group="+group+"&area="+area+"&device="+dev+"&sidx="+sidx+"&sord="+sord;
...
<table width="100%" class="formtable">
	<tr>
		...
		<td width="14%"> <?php echo $this->lang->line("Area"); ?> : <br><select name="area" id="area" class="select ui-widget-content ui-corner-all" ><option value=''><?php echo $this->lang->line("Please Select"); ?></option><?php echo $area; ?></select></td>
		<td width="14%"> <?php echo $this->lang->line("Group"); ?> : <br><select onchange="filterAssetsCombo(this.value,'device_area')" name="group" id="group_device_area" class="select ui-widget-content ui-corner-all" ><option value=''><?php echo $this->lang->line("Please Select"); ?></option><?php echo $group; ?></select></td>

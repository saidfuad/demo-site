...
jQuery().ready(function (){
	...
	jQuery("#allpoints_grid<?php echo time(); ?>").jqGrid({
		...
		datatype: "local",
		colNames:["<?php $this->lang->line("ID"); ?>",'<?php echo $this->lang->line("Datetime"); ?>','Status', '<?php echo $this->lang->line("Address"); ?>', '<?php echo $this->lang->line("Speed"); ?>', '<?php echo $this->lang->line("View_on_Map"); ?>'],
		colModel:[
...
function call_start_new_line(arr_i)
{
		...
		if(arr_i == 0){
			...
			image = new google.maps.MarkerImage(img, new google.maps.Size(20, 34), new google.maps.Point(0,0), new google.maps.Point(0, 34));
			$("#all_point_pBar").progressbar("value" , 100);
	...
	//alert("3->"+arr_i);
		...
		if(totalDir > totalDir_count)
		{
			...
			if(latArr.length != arr_i) //comment removed
				 setTimeout(function(){call_start_new_line(arr_i)},Timer_counter);//comment removed
			totalDir_count++;
...
<table border="5" width="100%" class="formtable" style="margin-bottom: 5px;">
	<tr>
		...
		<td width="20%">Group : <select onchange="AllPointfilterAssetsCombo(this.value,'all_points_device')" name="group" id="group_device_stopreport" class="select ui-widget-content ui-corner-all" ><?php echo $group1; ?></select></td>

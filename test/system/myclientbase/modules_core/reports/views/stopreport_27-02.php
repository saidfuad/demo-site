...
jQuery().ready(function (){
	...
	/**jQuery("#stopreport_grid").jqGrid("navButtonAdd","#stopreport_pager",{caption:"Pdf",
		onClickButton:function(){
			var sdate = $('#stop_sdate').val();
			var edate = $('#stop_edate').val();
			var group = $("#group_device_stopreport").val();
			...
			var qrystr ="/pdf?sdate="+sdate+"&edate="+edate+"&group="+group+"&stop_minute="+stop_minute+"&device="+dev;
			//document.location = "<?php echo site_url("reports/allpoints/export_pdf"); ?>"+qrystr, '_blank';
			window.open("<?php echo site_url('reports/stopreport/export_pdf'); ?>"+qrystr,'_blank');
			
			//var qrystr ="/pdf?sdate="+sdate+"&edate="+edate+"&device="+dev+"&sidx="+sidx+"&sord="+sord;
			//document.location = "<?php echo site_url("ignition_motion_report/export_pdf"); ?>" + qrystr;
			//window.open("<?php echo site_url("reports/battery/export_pdf"); ?>" + qrystr, '_blank');
		}
	});*/
	...
	/*added by ashwini
	 var btnTitle = 'Print Grid';
        ...
        $("#stopreport_grid").jqGrid('navSeparatorAdd','#stopreport_grid_toppager_left', {sepclass :'ui-separator'});
        $("#stopreport_grid").jqGrid('navButtonAdd','#stopreport_grid_toppager_left', {caption: '', title: btnTitle, position: 'last', buttonicon: 'ui-icon-print', onClickButton: function() {    PrintGrid();    } });

        // setup print button in the grid bottom navigation bar.
        $("#stopreport_grid").jqGrid('navSeparatorAdd','#stopreport_pager', {sepclass : "ui-separator"});
        $("#stopreport_grid").jqGrid('navButtonAdd','#stopreport_pager', {caption: 'Print Grid', title: btnTitle, position: 'last', buttonicon: 'ui-icon-print', onClickButton: function() { PrintGrid();    } });
        ...
        function PrintGrid(){
            ...
            $('#stopreport_grid').clone().appendTo('#prt-container').css({'page-break-after':'auto'});
            ...
            $('#prt-container').printElement({pageTitle:'Print', overrideElementCSS:[{href:'css/print-container.css',media:'print'}]});
...
function view_stop_report_map(id,asset){
	...
	$('#tabs ul.ui-tabs-nav li a').each(function(i) {
		if (this.text == nameToCheck) {
			...
			$('#tabs').tabs('add', '<?php echo base_url(); ?>index.php/reports/stopreport/view_map/0/id/'+id+'/asset/'+asset, '<?php echo $this->lang->line("Stop Report Map"); ?>');
			return false;
		}
	});
	if (!tabNameExists){
...
<table width="100%" class="formtable" style="margin-bottom: 5px;"  cellspacing="4" cellpadding="4">
	<tr>
		<td><?php echo $this->lang->line("Start"); ?> : <br><input type="text" name="sdate" id="stop_sdate" class="date text ui-widget-content ui-corner-all" style="width:165px" readonly="readonly"/></td>
		<td><?php echo $this->lang->line("End"); ?> : <br><input type="text" name="edate" id="stop_edate" class="date text ui-widget-content ui-corner-all" style="width:165px" readonly="readonly"/></td>
		<td><?php echo $this->lang->line("minutes"); ?> : <br><input type="text" name="stop_minute" id="stop_minute" class="text ui-widget-content ui-corner-all" style="width:50px"/></td>
		<td><?php echo $this->lang->line("Group"); ?> : <br><select onchange="filterAssetsCombo(this.value,'stop_reports_device')" name="group" id="group_device_stopreport" class="select ui-widget-content ui-corner-all" ><option value=''><?php echo $this->lang->line("Please Select"); ?></option><?php echo $group; ?></select></td>

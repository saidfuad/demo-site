...
jQuery().ready(function (){
	...
	jQuery("#stopreport_grid").jqGrid({
		...
		datatype: "local",
		colNames:["<?php echo $this->lang->line("ID"); ?>",'<?php echo $this->lang->line("Asset_Name"); ?>','<?php echo $this->lang->line("End_Date_Time"); ?>','<?php echo $this->lang->line("Start_Date_Time"); ?>', '<?php echo $this->lang->line("Duration"); ?>', '<?php echo $this->lang->line("Location"); ?>', '<?php echo $this->lang->line("View_on_Map"); ?>','Latitude','Longitude','Ignition Status'],
		colModel:[
			...
			{name:"ignition_off",editable:true, index:"ignition_off", width:170, align:"center", jsonmap:"ignition_off", formatter: 'date', formatoptions:{srcformat:"Y-m-d H:i:s",newformat:"<?php echo $date_format." ".$time_format; ?>"}},
			{name:"ignition_on",editable:true, index:"ignition_on", width:170, align:"center", jsonmap:"ignition_on",formatter: 'date', formatoptions:{srcformat:"Y-m-d H:i:s",newformat:"<?php echo $date_format." ".$time_format; ?>"}},
			{name:"duration",editable:true, index:"duration", width:70, align:"center", jsonmap:"duration"},
			{name:"address",editable:true, index:"address", width:165, align:"center", jsonmap:"address"},
			{name:"map",editable:true, index:"map", width:40, align:"center", jsonmap:"map",formatter:format_stop_map},
			{name:"lat",editable:true, index:"lat", width:85, align:"center", jsonmap:"lat"},
			{name:"lng",editable:true, index:"lng", width:85, align:"center", jsonmap:"lng"},
			{name:"ignition_status",editable:true, index:"ignition_status", width:115, align:"center", jsonmap:"ignition_status"}
	...
	jQuery("#stopreport_grid").jqGrid("navButtonAdd","#stopreport_pager",{caption:"<?php echo $this->lang->line("Export"); ?>",
		onClickButton:function(){
			...
			var ignitionstatus=$('#ignitionstatus option:selected').val();
			...
			if(dev == ''){
		 	...
		 	var qrystr ="/export?sdate="+sdate+"&edate="+edate+"&group="+group+"&stop_minute="+stop_minute+"&device="+dev+"&ignitionstatus"+ignitionstatus;
	...
	$("#stop_edate").datetimepicker({dateFormat:'<?php echo $js_date_format; ?>',timeFormat: '<?php echo $js_time_format; ?>',<?php echo $ampm; ?>changeMonth: true,showSecond: true,changeYear: true});
         ...
         $("#stop_sdate").datetimepicker('setDate', new Date('<?php echo date("Y/m/d H:i:s",strtotime("12:00:00 am")); ?>'));
	 $("#stop_edate").datetimepicker('setDate', new Date('<?php echo date("Y/m/d H:i:s",strtotime("11:59:59 pm")); ?>'));
...
function searchstopreport(){
	...
	jQuery("#stopreport_grid").jqGrid('setGridParam',{postData:{stop_minute:stop_minute, sdate:sdate,edate:edate, group:group, device:dev,ignitionstatus:ignitionstatus, page:1}}).trigger("reloadGrid");
	return false;
}
function cancel(){
...
function view_stop_report_map(id,asset){
	...
	$('#tabs ul.ui-tabs-nav li a').each(function(i) {
		if (this.text == nameToCheck) {
			...
			$('#tabs').tabs('add', '<?php echo base_url(); ?>index.php/reports/stopreport/view_map/0/id/'+id+'/asset/'+asset+'/ignitionstatus/'+ignitionstatus, 'Stop Report Map');
			return false;
		}
	});
	if (!tabNameExists){
...
<table width="100%" class="formtable" style="margin-bottom: 5px;"  cellspacing="5" cellpadding="5">
	<tr>
		<td><?php echo $this->lang->line("Start"); ?> : <br><input type="text" name="sdate" id="stop_sdate" class="date text ui-widget-content ui-corner-all" style="width:170px" readonly="readonly"/></td>
		<td><?php echo $this->lang->line("End"); ?> : <br><input type="text" name="edate" id="stop_edate" class="date text ui-widget-content ui-corner-all" style="width:170px" readonly="readonly"/></td>
		...
		<td>Ignition Status : <br><select onchange="" name="ignitionstatus" id="ignitionstatus" class="select ui-widget-content ui-corner-all" >
		<option value="<?php echo "both"; ?>"><?php echo $this->lang->line("both"); ?></option>
		<option value="<?php echo "ignition_on"; ?>"><?php echo $this->lang->line("ignition_on"); ?></option>
		<option value="<?php echo "ignition_off"; ?>"><?php echo $this->lang->line("ignition_off"); ?></option>

...
jQuery().ready(function (){
	...
	jQuery("#runreport_grid").jqGrid({
		...
		datatype: "local",
		colNames:["<?php echo $this->lang->line("ID"); ?>", '<?php echo $this->lang->line("Device"); ?>','<?php echo $this->lang->line("Start_Time"); ?>','<?php echo $this->lang->line("Stop_Time"); ?>', 'Start Odo.', 'Stop Odo.', '<?php echo $this->lang->line("running_time"); ?> (Min)', 'd1', '<?php echo $this->lang->line("Distance_KM"); ?>', '<?php echo $this->lang->line("Start Address"); ?>', '<?php echo $this->lang->line("Stop Address"); ?>', '<?php echo $this->lang->line("Max Speed"); ?>', '<?php echo $this->lang->line("Avg Speed"); ?>'],
		colModel:[
			{name:"id",index:"id",hidden:true, width:15, jsonmap:"id"},
			{name:"asset",editable:true, index:"asset", width:0, align:"center", jsonmap:"asset"},
			...
			{name:"ignition_off",editable:true, index:"ignition_off", width:150, align:"center", jsonmap:"ignition_off", formatter: 'date', formatoptions:{srcformat:"Y-m-d H:i:s",newformat:"<?php echo $date_format.' '.$time_format; ?>"}, summaryType:'count', summaryTpl:'<?php echo $this->lang->line("Total distance"); ?>'},
			{name:"start_odometer",editable:true, index:"start_odometer", width:70, align:"center", jsonmap:"start_odometer"},
			{name:"stop_odometer",editable:true, index:"stop_odometer", width:70, align:"center", jsonmap:"stop_odometer"},
			{name:"duration",editable:true, index:"duration", width:140, align:"center", jsonmap:"duration", summaryType:'sum'},
                        {name:"d1",editable:true, index:"d1", width:140, align:"center", jsonmap:"d1", formatter: 'date', formatoptions:{srcformat:"H:i:s",newformat:"H:i:s"},summaryType:'sum'},
			{name:"distance",editable:true, index:"distance", width:100, align:"center", jsonmap:"distance",formatter:'number', summaryType:'sum'},
                        {name:"ignition_on_address",editable:true, index:"ignition_on_address", width:150, align:"center", jsonmap:"ignition_on_address"},
                        {name:"ignition_off_address",editable:true, index:"ignition_off_address", width:150, align:"center", jsonmap:"ignition_off_address"},
                        {name:"max_speed",editable:true, index:"max_speed", width:100, align:"center", jsonmap:"max_speed"},
                        {name:"avg_speed",editable:true, index:"avg_speed", width:100, align:"center", jsonmap:"avg_speed"}
		],
		rowNum:grid_paging,
		height: 'auto',
		...
		groupingView : { 
			...
			groupText : ['<b>{0}</b>','{0} Sum of totaly: {d1}'],
			groupCollapse : false,
			groupSummary : [true],
	...
	/*jQuery("#runreport_grid").jqGrid("navButtonAdd","#runreport_pager",{caption:"<?php echo $this->lang->line("pdf"); ?>",
		onClickButton:function(){
			...
			document.location = "<?php echo base_url(); ?>index.php/reports/runreport/export_pdf"+qrystr;
		}
	});*/
	/*jQuery("#runreport_grid").jqGrid("navButtonAdd","#runreport_pager",{caption:"Pdf",
		onClickButton:function(){
			var sdate = $('#runreportsdate').val();
			var edate = $('#runreportedate').val();
			var group = $('#group_run_report').val();
			var qrystr ="/pdf?sdate="+sdate+"&edate="+edate+"&device="+group;
			
			window.open("<?php echo site_url('reports/runreport/export_pdf'); ?>"+qrystr,'_blank');
...
<table width="100%" class="formtable" style="margin-bottom: 5px;">
	<tr>
		...
		<td width="14%"><?php echo $this->lang->line("Group"); ?> : <select onchange="filterAssetsCombo(this.value,'runreportdevice')" style="width:120px;" name="group" id="group_run_report" class="select ui-widget-content ui-corner-all" ><option value=''><?php echo $this->lang->line("Please Select"); ?></option><?php echo $group; ?></select></td>

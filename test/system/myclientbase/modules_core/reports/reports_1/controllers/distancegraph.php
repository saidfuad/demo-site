			...
			for($i=0;$i<count($devices);$i++){
				...
				foreach($records as $rows){
					if($rows->add_date==$dates[$j] && $rows->devices==$devices[$i]){
						...
						$fuel_used = 0;
						$label = "<b>".$rows->distance." ".$this->lang->line("KM");
						
						if($rows->fuel_in_out_sensor == 1){
							$fuel_used = $rows->fuel_used;
							$label .= "<br><span style='color:red'>".$fuel_used." ".$this->lang->line("Ltr")."</span><span style='color:green'><br>".$this->lang->line("Avg").":".round($rows->distance/$fuel_used, 2)." ".$this->lang->line("Kmpl")."</span>";
						}
						$label .= "<b>";

	...
	function view_map(){
		...
		if(count($rows)){
			foreach ($rows as $row){
				...
				$stp_html .= "<tr><td>".$this->lang->line("Vehicle").' : '."</td><td>".$row->assets.' ('.$row->device_id.')</td></tr>';
	...
	function trackOnMap()
	{
		...
		/*	if($rows[$i]['ignition']!=0)
			{*/
				...
				$lat[] = $rows[$i]['lati'];
				$lng[] = $rows[$i]['longi'];
				$text = $this->lang->line("Date").' : '.date($date_format.' '.$time_format, strtotime($rows[$i]['add_date']))."<br>";
				$text .= $this->lang->line("Speed").' : '.$rows[$i]['speed']."<br>";
				//$text .= 'Lat : '.$row->lati.'<br>';
				//$text .= 'Lng : '.$row->longi.'<br>';
				$text .= $this->lang->line("Address").' : '.$rows[$i]['address'].'<br>';
				$html[] = $text;
				$ignition_status[]=1;
			/*}

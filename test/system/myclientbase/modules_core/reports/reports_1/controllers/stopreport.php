	...
	function loaddata($cmd='false')
	{
		...
		foreach($data['result'] as $row) {
			if($row->duration == ""){
				...
				if($h > 0)
					$stop_time .= $h + $hh." ".$this->lang->line("Hour")." ";
				if($m > 0)
				...
				if($duration[0] > 0)
					$stop_time .= intval($duration[0])." ".$this->lang->line("Hour")." ";
				if($duration[1] > 0)
					$stop_time .= intval($duration[1])." ".$this->lang->line("Min");
	...
	function view_map(){
		...
		if(count($rows)) {
			foreach ($rows as $row) {
				...
				$stp_html .= "<table><tr><td>".$this->lang->line("Device")." : </td><td>".$device."</td></tr>";
				...
				$stp_html .= "<tr><td>".$this->lang->line("Stop Duration")." : </td><td>".$row->duration."</td></tr>";
				$stp_html .= "<tr><td>".$this->lang->line("Latitude")." : </td><td>".$row->lat."</td></tr>";
				$stp_html .= "<tr><td>".$this->lang->line("Longitude")." : </td><td>".$row->lng."</td></tr>";
				$stp_html .= "</table>";
				$data['html'] = $stp_html;
			} 
		}

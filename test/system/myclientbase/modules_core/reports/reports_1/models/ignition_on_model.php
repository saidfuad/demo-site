...
class Ignition_on_model extends Model 
{
	/**
	* Instanciar o CI
	*/
	public function ignition_on_model()
    {
        parent::Model();
		$this->CI =& get_instance();
		$this->load->database();
		$this->table_name = "tbl_ignition_on_speed_off";
    }
	...
	public function get_stopdata($cmd) 
	{
		...
		$SQL = "SELECT count(*) as total FROM ".$this->table_name." tm WHERE CONVERT_TZ(tm.motion_stop_time,'+00:00','".$this->session->userdata('timezone')."') BETWEEN '" . $sdate . "' AND '".$edate."' and tm.motion_start_time is not null ";
		...
		//$SQL = "SELECT tm.id, am.device_id, CONVERT_TZ(tm.motion_stop_time,'+00:00','".$this->session->userdata('timezone')."') as motion_stop_time, CONVERT_TZ(tm.motion_start_time,'+00:00','".$this->session->userdata('timezone')."') as motion_start_time, tm.duration, tm.address, am.assets_name FROM ".$this->table_name." tm left join assests_master am on tm.device_id=am.id WHERE CONVERT_TZ(tm.motion_stop_time,'+00:00','".$this->session->userdata('timezone')."') BETWEEN '" . $sdate . "' AND '".$edate."' and motion_start_time is not null";
		$SQL = "SELECT tm.id, am.device_id, CONVERT_TZ(tm.motion_stop_time,'+00:00','".$this->session->userdata('timezone')."') as motion_stop_time, CONVERT_TZ(tm.motion_start_time,'+00:00','".$this->session->userdata('timezone')."') as motion_start_time, tm.duration, tm.address, am.assets_name FROM ".$this->table_name." tm left join assests_master am on tm.device_id=am.id WHERE CONVERT_TZ(tm.motion_stop_time,'+00:00','".$this->session->userdata('timezone')."') BETWEEN '" . $sdate . "' AND '".$edate."' and duration NOT LIKE '%-%' and duration!='00:00:00' and motion_start_time is not null";
		if($device){	//search by device
			$SQL .= " AND find_in_set(tm.device_id,'$device')";
		}else{
			$SQL .= " AND tm.device_id in (select id from assests_master where find_in_set(id, (SELECT assets_ids FROM user_assets_map where user_id = $user)))";		
		...
		if($cmd=="export") 
		{
			...
			header("Content-Disposition: attachment; filename=ignition_on_". date("s").".xls"); 
			$EXCEL = "";
			$fitr="";
			...
			foreach($result->result_array() as $data)
				{
					$motion_stop_time = $data['motion_stop_time'];
					$motion_start_time = $data['motion_start_time'];
					//$add_date = $data['add_date'];
					$EXCEL .="<tr align='center'>";
					$EXCEL.="<td>".$data['assets_name']."(".$data['device_id'].")</td>"; 
					$EXCEL.="<td>&nbsp;".date("$date_format $time_format", strtotime($motion_stop_time))."</td>"; 
					$EXCEL.="<td>&nbsp;".date("$date_format $time_format", strtotime($motion_start_time))."</td>"; 
					$EXCEL.="<td>".$data['address']."</td>";
					$EXCEL.="<td>".$data['duration']."</td>";
					$EXCEL .="</tr>";
					$date=date("$date_format $time_format", strtotime($motion_stop_time));
			...
			if($device == '')
 			...
 			echo "<table border='1'>";
			echo "<tr><th colspan='5'>".$this->lang->line("Ignition On(Vehicle in stop mode)")."</th></tr>";
			echo "<tr><th colspan='2'>".$this->lang->line("From")."</th><th colspan='2'>".$this->lang->line("To")."</th><th>".$this->lang->line("Assets Name")."</th></tr>";
	...
	*/
	...
	function get_map_data(){
			$id=uri_assoc('id');
			$squery="select id, device_id, CONVERT_TZ(motion_stop_time,'+00:00','".$this->session->userdata('timezone')."') as motion_stop_time, CONVERT_TZ(motion_start_time,'+00:00','".$this->session->userdata('timezone')."') as motion_start_time, duration, address, lat, lng, CONVERT_TZ(add_date,'+00:00','".$this->session->userdata('timezone')."') as add_date from ".$this->table_name." where id=$id Limit 1";

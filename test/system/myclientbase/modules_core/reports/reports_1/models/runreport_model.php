	...
	*/
	...
	//	$cmd=uri_assoc('cmd');
		...
		*/
		...
		$SQL = "SELECT dm.id, CONCAT(am.assets_name,' (',am.device_id,')',' (Mileage - ',am.km_reading,')') as asset, dm.ignition_on, dm.ignition_off, dm.duration, dm.start_odometer, dm.stop_odometer, dm.distance from tbl_start_report dm left join assests_master am on am.id = dm.device_id WHERE ";
		if($this->session->userdata("usertype_id")!=1){
			$SQL .=  " am.id in (select id from assests_master where find_in_set(id, (SELECT assets_ids FROM user_assets_map where user_id = $user))) and ";
		}
		$SQL .= " dm.add_date BETWEEN '" . $sdate . "' AND '" . $edate . "'";
		...
		$export_sql="";
		$export_sql=$SQL;
		$SQL .= " ORDER BY $sidx $sord, dm.add_date ASC LIMIT $start, $limit";
		...
		if($cmd=="export")   
		{
			...
			echo "<tr><th colspan='3'>".$this->lang->line("Device")."Distance</th></tr>";

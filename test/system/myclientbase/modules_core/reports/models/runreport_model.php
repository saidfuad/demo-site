	...
	*/
	...
	//	$cmd=uri_assoc('cmd');
		...
		*/
		...
		$SQL = "SELECT dm.id,CASE WHEN am.driver_name IS NOT NULL and am.driver_name != '' THEN CONCAT(am.assets_name,' (',am.device_id,')',' (Mileage - ',am.km_reading,')',' (Driver Name  - ',am.driver_name,')') ELSE CONCAT(am.assets_name,' (',am.device_id,')',' (Mileage - ',am.km_reading,')') END as asset , CONCAT(am.assets_name,' (',am.device_id,')') as asset_name, dm.ignition_on, dm.ignition_off, ROUND((TIME_TO_SEC(dm.duration) / 60), 0) as duration , dm.duration as d1 ,dm.ignition_on_address,dm.ignition_off_address,dm.max_speed,dm.avg_speed, dm.start_odometer, dm.stop_odometer, dm.distance from tbl_start_report dm left join assests_master am on am.id = dm.device_id WHERE ";

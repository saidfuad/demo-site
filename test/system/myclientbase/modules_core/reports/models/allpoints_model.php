...
class Allpoints_model extends Model 
{
	...
	*/
	...
	//	die($sdate."->".$edate);
		...
		//die($assets_nm);
		$SQL = "SELECT tm.id, CONVERT_TZ(tm.add_date,'+00:00','".$this->session->userdata('timezone')."') as add_date, '$assets_nm' as assets_name, tm.device_id, tm.lati, tm.longi, tm.speed, tm.address FROM ".$this->table_name." tm WHERE CONVERT_TZ(tm.add_date,'+00:00','".$this->session->userdata('timezone')."') BETWEEN '" . $sdate . "' AND '" . $edate . "'";
		...
		if($cmd=="export") 
		{
			...
			echo "<table border='1'>";
			echo "<tr><th colspan='4'> All points</th></tr>";
...
/*			
		$format = $this->session->userdata('timezone');
		$sdate = new DateTime($sdate);
		$edate = new DateTime($edate);
		if(substr($format, 0,1) == '+'){
			$hours = explode(':', substr($format,1))[0];
			$mins = explode(':',  substr($format, 1))[1];
			$sdate->sub(new DateInterval('PT'.$hours.'H'.$mins.'M'));
			$edate->sub(new DateInterval('PT'.$hours.'H'.$mins.'M'));
		}
		else if(substr($format,0,1) == '-'){
			$hours = explode(':',substr($format, 1))[0];
			$mins = explode(':', substr($format, 1))[1];
			$sdate->add(new DateInterval('PT'.$hours.'H'.$mins.'M'));
			$edate->add(new DateInterval('PT'.$hours.'H'.$mins.'M'));
		}
		else{
			$hours = explode(':', $format)[0];
			$mins = explode(':', $format)[1];
			$sdate->sub(new DateInterval('PT'.$hours.'H'.$mins.'M'));
			$edate->sub(new DateInterval('PT'.$hours.'H'.$mins.'M'));
		}
*/		
		//$SQL = "SELECT count(*) FROM ".$this->table_name." tm left join ".$this->tbl_assets." am on am.device_id = tm.device_id WHERE am.status=1 AND am.del_date is null AND date(tm.add_date) BETWEEN '" . $sdate . "' AND '" . $edate . "'";
		$SQL = "SELECT count(1) as total FROM ".$this->table_name." tm use index (dt, assets_id, speed) WHERE tm.dt  BETWEEN '" . $sdate . "' AND '" . $edate . "' ";
		...
		$assets_nm = $query12[0]['assets_name'];
		//die($assets_nm);
		$SQL = "SELECT tm.id, tm.dt as add_date, '$assets_nm' as assets_name, tm.device_id, tm.lati, tm.longi, tm.speed, tm.address, tm.driver_name FROM ".$this->table_name." tm use index(dt, assets_id, speed) WHERE tm.dt BETWEEN '" . $sdate . "' AND '" . $edate . "'";
		//$SQL = "SELECT tm.id, tm.add_date, '' as assets_name, tm.device_id, tm.lati, tm.longi, tm.speed, tm.address FROM ".$this->table_name." tm WHERE tm.add_date BETWEEN '" . $sdate . "' AND '" . $edate . "'";
		if($device)	//search by devices
			$SQL .= " AND tm.assets_id = $device";
	...
	//New function to show points on map
	public function get_all_locations_new() 
	{
		//Select table name
		$device = $this->input->post('device');
		$sdate = $this->input->post('start_date');
		$edate = $this->input->post('end_date');
		$user_tz=$this->session->userdata('timezone');
		$qry_rs="SELECT id, lati, longi, phone_imei, dt as add_date, speed, device_id, dt, ignition, address, odometer FROM ".$this->table_name." use index(dt, assets_id, speed) WHERE ";
		if($sdate && $edate){	//search by date
			$sdate = date("Y-m-d H:i:s", strtotime($sdate));
			$edate = date("Y-m-d H:i:s", strtotime($edate));
		}else{
...
public function get_assets_category() 
	{
		...
		$device = $this->input->post('device');
		$SQL = "select assets_category_id from ".$this->tbl_assets;
		$SQL .= " where id = $device";

	...
	//	$cmd=uri_assoc('cmd');
		...
		*/
		...
		if($cmd=="export")
		{
			...
			$fitr.="<th>1st Vehicle</th>";
			$fitr.="<th>2nd Vehicle</th>";
			...
			echo "<table border='1'>";
			$top="";
			$top .= "<tr><th colspan='4'>Distance</th></tr>";
			$top .= "<tr><th colspan='1'>Start Date</th><th colspan='1'>End Date</th><th colspan='2'>Assets Name</th></tr>";
		...
		*/
		$device_name='';
			...
			$result = $this->db->query($export_sql);
			if($device!=""){
			$ids = $temp_as = implode(',', $device);
				$SQLAssets = $this->db->query("select assets_name from assests_master where id in($device) ");
				$rs=$SQLAssets->result();
				//print_r($rs);
				$assetName=array();
				//print_r($devName);
			...
			$top .= "<tr><th colspan='4'>Distance</th></tr>";
			$top .= "<tr><th colspan='1'>Start Date</th><th colspan='1'>End Date</th><th colspan='2'>Assets Name</th></tr>";
			$top .= "<tr><th colspan='1'>&nbsp;".date("$date_format", strtotime($sdate))."</th><th colspan='1'>&nbsp;".date("$date_format", strtotime($edate))."</th><th colspan='2'>".$devName."</th></tr>";
			echo $top;
			echo $fitr;
			echo $EXCEL;
			echo "</table>";

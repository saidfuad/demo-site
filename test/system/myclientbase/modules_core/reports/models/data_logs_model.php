...
class data_logs_model extends Model 
{
	...
	*/
	...
	function getAllData(){
		...
		$SQL = "SELECT assets_name FROM assests_master WHERE device_id = '$device' and status = 1 and del_date IS NULL";
		
		$result = $this->db->query($SQL);

		$as_row = $result->result_array();
		$assets_name = $as_row[0]['assets_name'];
		...
		if($cmd == 'export') {
			...
			foreach($result->result_array() as $data)
				...
				echo "<table border='1'>";
				echo "<tr><th colspan='2'> Data Logs for Device : $assets_name</th></tr>";
				echo $fitr;
				echo $EXCEL;
				echo "</table>";
				die(); 

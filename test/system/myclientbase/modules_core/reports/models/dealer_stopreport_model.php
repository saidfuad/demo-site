...
class Dealer_stopreport_model extends Model 
{
	/**
	* Instanciar o CI
	*/
	public function Dealer_stopreport_model()
    {
        parent::Model();
	...
	public function get_stopdata($cmd) 
	{
		...
		//$SQL = "SELECT count(*) as total FROM ".$this->table_name." tm WHERE tm.ignition_off BETWEEN '" . $sdate . "' AND '".$edate."' and tm.ignition_on is not null and current_landmark is not null";
		$SQL = "SELECT count(*) as total FROM ".$this->table_name." tm left join assests_master am on tm.device_id=am.id WHERE CONVERT_TZ(tm.ignition_off,'+00:00','".$this->session->userdata('timezone')."') BETWEEN '" . $sdate . "' AND '".$edate."' and ignition_on is not null and current_area is not null and am.id in (select id from assests_master where find_in_set(id, (SELECT assets_ids FROM user_assets_map where user_id = $user)))";
		...
		//$SQL = "SELECT tm.id, tm.device_id, tm.ignition_off, tm.ignition_on, tm.duration, tm.address, tm.current_area, am.assets_name FROM ".$this->table_name." tm left join assests_master am on tm.device_id=am.id WHERE tm.ignition_off BETWEEN '" . $sdate . "' AND '".$edate."' and ignition_on is not null and current_area is not null and current_area!=''";
		$SQL = "SELECT tm.id, am.device_id, CONVERT_TZ(tm.ignition_off,'+00:00','".$this->session->userdata('timezone')."') as ignition_off, CONVERT_TZ(tm.ignition_on,'+00:00','".$this->session->userdata('timezone')."') as ignition_on, tm.duration, tm.address, tm.current_area, am.assets_name FROM ".$this->table_name." tm left join assests_master am on tm.device_id=am.id WHERE CONVERT_TZ(tm.ignition_off,'+00:00','".$this->session->userdata('timezone')."') BETWEEN '" . $sdate . "' AND '".$edate."' and ignition_on is not null and current_area is not null and am.id in (select id from assests_master where find_in_set(id, (SELECT assets_ids FROM user_assets_map where user_id = $user)))";
		...
		if($cmd=="export") 
		{
			...
			header("Content-Disposition: attachment; filename=Dealer_Stop_Report". date("s").".xls"); 
			$EXCEL = "";
			$fitr="";
	...
	*/
	...
	function get_map_data(){
		$id=uri_assoc('id');
		$query = $this->db->query("select id, device_id, CONVERT_TZ(ignition_off,'+00:00','".$this->session->userdata('timezone')."') as ignition_off, CONVERT_TZ( ignition_on,'+00:00','".$this->session->userdata('timezone')."') as ignition_on, duration, address, lat, lng, CONVERT_TZ(add_date,'+00:00','".$this->session->userdata('timezone')."') as add_date, alert_given, current_area, current_landmark from ".$this->table_name." where id=$id Limit 1");
		return $query->result();	
	}
	function get_map_data_area($area){
		$user=$this->session->userdata('user_id');
		$query = $this->db->query("select * from areas where polyname='$area' and Audit_Enter_uid=$user and Audit_Del_Dt is null");
		return $query->result();
	}
}
?>
...
jQuery().ready(function (){
	...
	jQuery("#ignition_on_grid").jqGrid({
		...
		datatype: "local",
		colNames:["<?php echo $this->lang->line("ID"); ?>",'<?php echo $this->lang->line("Asset_Name"); ?>','Motion Stop Time','Motion Start Time', '<?php echo $this->lang->line("Duration"); ?>', 'Map'],
		colModel:[
...
function format_ignition_on_map(cellVal, options, rowObject){
	return "<a href='#' onclick='view_ignition_on_report_map("+rowObject.id+",\""+rowObject.assets_name+"\")'> <img src='<?php echo base_url(); ?>assets/marker-images/mini-RED-BLANK.png'></a>";
}
...
function view_ignition_on_report_map(id,asset){
	var nameToCheck = "Ignition Report Map";
	...
	$('#tabs ul.ui-tabs-nav li a').each(function(i) {
		if (this.text == nameToCheck) {
			...
			$('#tabs').tabs('add', 'reports/ignition_on/view_map/0/id/'+id+'/asset/'+asset, 'Ignition Report Map');
			return false;
		}
	});
	if (!tabNameExists){

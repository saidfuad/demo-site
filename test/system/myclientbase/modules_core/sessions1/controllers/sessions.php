...
class Sessions extends Controller {
	...
	function login() {
		...
		if ($this->agent->is_referral()) {
		    if('http://omexsol.com/' == $this->agent->referrer()) {
...
//				$ip = '117.206.145.69'; 
	...
	function logout(){  
		...
		$diff_time = strtotime(gmdate('Y-m-d H:i:s'))-strtotime($endDate);
		$datanya=array('last_logout_time'=>gmdate("Y-m-d H:i:s"),'duration_of_stay'=>"$diff_time");

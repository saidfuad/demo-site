...
class Home_model extends Model 
{
	...
	*/
	...
	public function get_last_20_points_report() 
	{
		...
		$SQL = "SELECT lp.id, CONVERT_TZ(lp.add_date,'+00:00','".$this->session->userdata('timezone')."') as add_date, lp.address, lp.ignition, lp.speed, lp.lati, lp.longi, am.assets_name FROM ".$this->table_name." lp LEFT JOIN ".$this->tbl_assets." am ON am.device_id = lp.device_id WHERE lp.assets_id = '$device' ORDER BY lp.id DESC LIMIT 0, 20";
	...
	public function get_devices($user) 
	{
		...
		if($this->session->userdata('user_id')!=1){
			$query = $this->db->query("select id, assets_name, device_id, assets_friendly_nm from assests_master where status=1 AND del_date is null AND find_in_set(id, (SELECT assets_ids FROM user_assets_map where user_id = $user))");
		}else{
			$query = $this->db->query("select id, assets_name, device_id, assets_friendly_nm from assests_master where status=1 AND del_date is null ");
	...
	{
		$query = $this->db->query("SELECT lm.*, (select group_concat(concat(assets_name,'(',device_id,')')) assets from assests_master where find_in_set(device_id, lm.device_ids)) as assets FROM `landmark` lm where lm.add_uid = $user and lm.del_date is null and lm.status = 1", FALSE);
		return $query->result();
	}
	...
	public function get_landmarks($user) 
	{
		if(count($user) > 0) {
			...
			$user_ids = implode(',', $user);
			$q = "SELECT id, name FROM landmark WHERE status=1 AND del_date is null AND add_uid IN($user_ids) ORDER BY name ASC ";
			$query = $this->db->query($q);
			return $query->result();
		}
		return false;

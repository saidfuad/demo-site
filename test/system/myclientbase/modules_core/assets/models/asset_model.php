...
class Asset_model extends Model 
{
	...
	*/
	...
	public function prepare_driver(){
		$user = $this->session->userdata('user_id');
		$this->db->select('*');
	...
	function save($db_array, $id, $set_flashdata = TRUE) {
            
        //$xyz= $db_array[asset_users_id];
        // echo"<pre>";
        //  print_r($xyz);
               
        $usr_array=array();
        $usr_array=explode(',',$db_array[asset_users_id]);
        $assets_group_id=$db_array[assets_group_id];
               //   echo"<pre>";
              // print_r($usr_array);
		...
		if($id==""){
			...
			foreach ($query->result() as $row) {
                ...
                if($row->group_id != "") {
					...
					if(! in_array($assets_group_id, $temp_as)) {
						$upd = 'UPDATE user_assets_map SET group_id = concat(group_id, ",'.$assets_group_id.'") WHERE id='.$row->id;
						$this->db->query($upd);
					}
				}else{
					$upd = 'UPDATE user_assets_map SET group_id = '.$assets_group_id.' WHERE id='.$row->id;
					$this->db->query($upd);
				}
			}
            foreach($usr_array as $usr_id)
            {
                // print_r($usr_id);
                if($usr_id!=""){
                    $sql = "SELECT * FROM `user_assets_map` where user_id = $usr_id";
                ...
                $this->db->where('id', $id);
                 $this->db->update("assests_master", $db_array);
                   $sql = "SELECT * FROM `user_assets_map` where user_id = $user";
		...
		foreach ($query->result() as $row) {
			if($row->assets_ids != "") {
				$temp_as = explode(',', $row->assets_ids);
				if(! in_array($insert_id, $temp_as)) {
					$upd = 'UPDATE user_assets_map SET assets_ids = concat(assets_ids, ",'.$id.'") WHERE id='.$row->id;
					$this->db->query($upd);
				}
			}else{
				$upd = 'UPDATE user_assets_map SET assets_ids = '.$id.' WHERE id='.$row->id;
				$this->db->query($upd);
			}
                        if($row->group_id != "") {
				$temp_as = explode(',', $row->group_id);
...
*/
  ...
  $id=uri_assoc('id');
  $query="select mobile_no from driver_master where driver_name='$id'";
  ...
  foreach($data->result() as $row)
	...
	public function assets_group_data(){
		...
		if($this->session->userdata('user_id') != 1) {
			//$query = "SELECT gm.id, gm.group_name FROM group_master gm, user_assets_map um, tbl_users as tu WHERE (find_in_set(gm.id, um.group_id ) AND um.user_id = ".$this->session->userdata('user_id').")or(find_in_set(tu.user_id, gm.add_uid ) AND tu.user_id = ".$this->session->userdata('user_id').") AND gm.status = 1 ORDER BY gm.group_name";
...
*/		
	...
	public function prepare_owner_combo(){
		...
		$query = "select id, owner from `assests_owner_master` where status= '1' AND del_date is Null AND add_uid = ".$this->session->userdata('user_id');
		$data  = $this->db->query($query);

	//	echo "<option value='' >Please Select</option>";
		$ownerOpt="<option value='' >Please Select</option>";
	...
	public function prepare_division_combo(){
		...
		$divisionOpt = "<option value='' >Please Select</option>";

		$query = "select id, division from `assests_division_master` where status= '1' AND del_date is Null AND add_uid = ".$this->session->userdata('user_id');
		$data  = $this->db->query($query);
	...
	public function map_assets() {
	 ...
	 function get_asset_users(){
           $query ="select user_id,username from tbl_users where del_date is null and status=1 and admin_id=".$this->session->userdata('user_id');
          // $query ="select user_id,username from tbl_users where del_date is null and status=1 ORDER by username ASC";

...
jQuery().ready(function (){ 
	...
	jQuery("#history_grid").jqGrid({
		...
		datatype: "json",
		colNames:["<?php echo $this->lang->line("ID"); ?>",'<?php echo $this->lang->line("Last Login Date & Time"); ?>','<?php echo $this->lang->line("Last Logout Date & Time"); ?>','<?php echo $this->lang->line("IP Address"); ?>','<?php echo $this->lang->line("Country Name"); ?>','<?php echo $this->lang->line("State Name"); ?>','<?php echo $this->lang->line("City Name"); ?>','<?php echo $this->lang->line("Os Name"); ?>','<?php echo $this->lang->line("Device"); ?>','<?php echo $this->lang->line("Duration Of Stay(In Min.)"); ?>','<?php echo $this->lang->line("Langitute"); ?>','<?php echo $this->lang->line("Longitute"); ?>','<?php echo $this->lang->line("View_on_Map"); ?>'],
		colModel:[
...
function search_history(){
	...
	var sdate = $('#sdate_history').val();
	var edate = $('#edate_history').val();
	...
	jQuery("#history_grid").jqGrid('setGridParam',{postData:{sdate:sdate, edate:edate,  page:1}}).trigger("reloadGrid");
	return false;	
}
...
function view_map(id){
	$('#tabs').tabs('add', "<?php echo base_url(); ?>index.php/history/view_map?cmd=history&id="+id, 'View Login ', 1);
...
<table width="100%">
	<tr>
		<td width="20%"><?php echo $this->lang->line("Start"); ?> : <input type="text" name="sdate_history" id="sdate_history" class="date text ui-widget-content ui-corner-all" style="margin-top: 5px;padding: 0.4em;width: 54%;" value="<?php echo date('1.m.Y'); ?>" readonly="readonly"/></td>
		<td width="20%"><?php echo $this->lang->line("End"); ?> : <input type="text" name="edate_history" id="edate_history" class="date text ui-widget-content ui-corner-all" style="margin-top: 5px;padding: 0.4em;width: 54%;" value="<?php echo date('d.m.Y'); ?>" readonly="readonly"/></td>

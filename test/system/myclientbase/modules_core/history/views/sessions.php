...
class Sessions extends Controller {
	...
	function login() {
		...
		if ($this->mdl_sessions->validate()) {
			...
			if ($user = $this->mdl_auth->auth('tbl_users', 'username', 'password', $this->input->post('username'), $this->input->post('password'))) {
				
				$object_vars = array('user_id', 'last_name','email_address','mobile_number', 'first_name', 'global_admin', 'admin_id', 'company_name','usertype_id','language','timezone','date_format','time_format','currency_format','language','photo');
...
//				$ip = '117.206.145.69'; 
		//		die($ip);
					...
					if($this->session->userdata('admin_id') != "")
					{
						...
						$last_name = $this->session->userdata('last_name');
							...
							$array=array(
								...
								'alert_msg'    =>   'User Name : '.$first_name.' '.$last_name.'<br> Login Time : '.$dates,
								'alert_link'   =>  '',
								'alert_type'   =>  'alert',
								'user_id'	   =>  $user_id,
								'add_date'     =>   $datesa 					
				...
				//	$ip = '59.95.198.247'; 
					...
					$data['password']=$this->input->post('password');
					$datanya=array('ip_address'=>$ip,'country_name'=>$data['CountryName'],'state_name'=>$data['RegionName'],'city_name'=>$data['City'],'os_name'=>$user_os,'device'=>$user_browser,'latitude'=>$data['Latitude'],'longitude'=>$data['Longitude'],'user_id'=>$id,'add_date'=>date("Y-m-d H:i:s"),'add_uid'=>$id,'status'=>'1','comments'=>'');

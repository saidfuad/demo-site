	...
	*/
	...
	function get_map_data(){
		...
		if($_REQUEST['cmd']=='history')
		{
			...
			$query = $this->db->query("select id, ip_address, country_name, state_name, city_name, os_name, device, latitude, longitude, last_login_time, last_logout_time, duration_of_stay, user_id, add_date, add_uid, del_date, del_uid, status, comments from sys_information where id = ".$id);
			return $query->result();	
	...
	function getAllData(){
		...
		$SQL = "SELECT * FROM sys_information WHERE last_login_time between '$sdate' and '$edate' and user_id = ".$this->session->userdata('user_id');
		...
		$SQL = "SELECT id, ip_address, country_name, state_name, city_name, os_name, device, latitude, longitude, last_login_time, last_logout_time, duration_of_stay, user_id, CONVERT_TZ(add_date,'+00:00','".$this->session->userdata('timezone')."') as add_date, add_uid, del_date, del_uid, status, comments FROM sys_information WHERE last_login_time between '$sdate' and '$edate' and user_id = ".$this->session->userdata('user_id');
		if($where != "")      
			$SQL .= " AND $where";
			//die($SQL);

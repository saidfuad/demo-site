...
class Fuel_report_model extends Model 
{
	/**
	* Instanciar o CI
	*/
	public function fuel_report_model()
    {
        parent::Model();
	...
	public function get_fueldata($cmd) 
	{
		...
		$SQL = "SELECT count(tm.id) as total FROM ".$this->table_name." tm left join assests_master am on tm.assets_id=am.id WHERE CONVERT_TZ(tm.add_date,'+00:00','".$this->session->userdata('timezone')."') BETWEEN '" . $sdate . "' AND '".$edate."' and fuel_reading <> '18' and fuel_percent is not null";
		...
		if($fuel_status == 1){
			$SQL .= " AND tm.fuel_litters < -2";
		}else if($fuel_status == 2){
			$SQL .= " AND tm.fuel_litters > 2";
		}else{
			$SQL .= " AND (tm.fuel_litters > 2 or tm.fuel_litters < -2)";
		}
		if($where != "")
			$SQL .= " AND $where";

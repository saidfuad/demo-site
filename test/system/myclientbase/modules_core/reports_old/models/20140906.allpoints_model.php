...
class Allpoints_model extends Model 
{
	...
	*/
	...
	//	die($sdate."->".$edate);
		...
		//die($assets_nm);
		$SQL = "SELECT tm.id, CONVERT_TZ(tm.add_date,'+00:00','".$this->session->userdata('timezone')."') as add_date, '$assets_nm' as assets_name, tm.device_id, tm.lati, tm.longi, tm.speed, tm.address FROM ".$this->table_name." tm WHERE tm.speed > 10 AND CONVERT_TZ(tm.add_date,'+00:00','".$this->session->userdata('timezone')."') BETWEEN '" . $sdate . "' AND '" . $edate . "'";
	...
	public function get_all_locations() 
	{
		...
		$user_tz=$this->session->userdata('timezone');
		$qry_rs="SELECT id, lati, longi, phone_imei, CONVERT_TZ(add_date,'+00:00','$user_tz') as add_date, speed, device_id, dt, ignition, address, odometer FROM ".$this->table_name." WHERE speed > 10 AND ";

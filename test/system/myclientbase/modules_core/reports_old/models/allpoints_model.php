...
class Allpoints_model extends Model 
{
	...
	*/
	...
	//	die($sdate."->".$edate);
		...
		/*ashwini
		$query="SELECT rfid FROM tbl_track where assets_id = $device ";

		$result_arr = $this->db->query($query);
	    $rfid_result = $result_arr->result_array();
	    $driver_rfid = $rfid_result[0]['rfid'];
		$sql_query="SELECT driver_name FROM driver_master where driver_code = $driver_rfid";
		
		$query_result = $this->db->query($sql_query);
	    $qresult = $query_result->result();
		$driver_name = $qresult['driver_name'];
		
		
		//$data['driver_details'] = $query_result->result();
		
		end*/
		
		
		//$SQL = "SELECT tm.id, CONVERT_TZ(tm.add_date,'+00:00','".$this->session->userdata('timezone')."') as add_date, '$assets_nm' as assets_name, tm.device_id, tm.lati, tm.longi, tm.speed, tm.address,tm.driver_name FROM ".$this->table_name." tm WHERE CONVERT_TZ(tm.add_date,'+00:00','".$this->session->userdata('timezone')."') BETWEEN '" . $sdate . "' AND '" . $edate . "'";
		$SQL = "SELECT tm.id, CONVERT_TZ(tm.add_date,'+00:00','".$this->session->userdata('timezone')."') as add_date, '$assets_nm' as assets_name, tm.device_id, tm.lati, tm.longi, tm.speed, tm.address,tm.driver_name FROM ".$this->table_name." tm WHERE CONVERT_TZ(tm.add_date,'+00:00','".$this->session->userdata('timezone')."') BETWEEN '" . $sdate . "' AND '" . $edate . "'";
	...
	public function prepareCombo(){
		...
		foreach ($query->result() as $row) {
			  ...
			  $a = "<option value='".$row->device_id."'>".$row->assets_name." (".$row->device_id.")</option>";
			 // echo $a;exit;

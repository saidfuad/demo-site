...
class Lastpoint_model extends Model 
{
	...
	*/
	...
	//	echo $sub."<br/>".$srch."<br/>".$stsWhr;
		...
		$SQL = "select tlp.id, CONVERT_TZ(tlp.add_date,'+00:00','".$this->session->userdata('timezone')."') as add_date, am.id as assets_id, am.assets_name, tlp.device_id, tlp.lati, tlp.longi, tlp.speed, tlp.address, tlp.runtime, tlp.data_type, tlp.battery_status, tlp.alarm_type, '' as map, TIME_TO_SEC(TIMEDIFF(NOW( ) , CONVERT_TZ(tlp.add_date,'+00:00','".$this->session->userdata('timezone')."'))) as beforeTime,(TIMESTAMPDIFF(SECOND,tlp.add_date,NOW())*60) as mins, tlp.current_area as in_area, tlp.current_landmark as near_landmark,tlp.cross_speed as cross_speed from assests_master am left join tbl_last_point tlp on tlp.device_id = am.device_id where am.status=1 AND am.del_date is null $gsub $usub $srch $stsWhr ";

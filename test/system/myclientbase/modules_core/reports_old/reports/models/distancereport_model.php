	...
	*/
	...
	//	$cmd=uri_assoc('cmd');
		...
		*/
		...
		$SQL .= " date(dm.add_date) BETWEEN '" . $sdate . "' AND '" . $edate . "'";
		...
		if($group != ""){
			$SQL .=" AND find_in_set(dm.assets_id, (select assets from group_master where id = $group))";
		}
		...
		$SQL = "SELECT dm.id,CONCAT(am.assets_name,'(',am.device_id,')') as assets_id, am.id as aId, CONVERT_TZ(dm.add_date,'+00:00','".$this->session->userdata('timezone')."') as add_date, dm.distance, dm.running_time from distance_master dm left join assests_master am on am.id=dm.assets_id WHERE ";
		if($this->session->userdata("usertype_id")!=1){
			$SQL .=  " am.id in (select id from assests_master where find_in_set(id, (SELECT assets_ids FROM user_assets_map where user_id = $user))) and ";
		}
		$SQL .= " date(dm.add_date) BETWEEN '" . $sdate . "' AND '" . $edate . "'";
		...
		if($cmd=="export")   
		{
			...
			$fitr.="<th>".$this->lang->line("Device")."</th>";
			$fitr .="</tr>"; 
			//var_dump($result);
			...
			echo "<tr><th colspan='1'>".$this->lang->line("Start Date")."</th><th colspan='1'>".$this->lang->line("End Date")."</th><th colspan='1'>".$this->lang->line("Assets Name")."</th></tr>";

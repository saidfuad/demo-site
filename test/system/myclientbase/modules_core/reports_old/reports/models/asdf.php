...
class Area_in_out_model extends Model 
{
	...
	*/
	...
	function getAllData($cmd){
		...
		$SQL .= " AND CONVERT_TZ(tm.date_time,'+00:00','".$this->session->userdata('timezone')."') BETWEEN '" . $sdate . "' AND '" . $edate . "'";
		...
		$SQL = "SELECT distinct(tm.id), CONVERT_TZ(tm.date_time,'+00:00','".$this->session->userdata('timezone')."') as date, tm.inout_status as status, am.assets_name as device, ta.polyname as area FROM ".$this->table_name." as tm left join ".$this->tbl_assets." am on am.id = tm.device_id left join ".$this->tbl_area." as ta on tm.area_id=ta.polyid WHERE 1 $sub"; 
		$SQL .= " AND CONVERT_TZ(tm.date_time,'+00:00','".$this->session->userdata('timezone')."')  BETWEEN '" . $sdate . "' AND '" . $edate . "'";
		if($area != ""){
			$SQL .=" AND tm.area_id = '$area'";
		}
	...
	{
		$query = $this->db->query("select * from areas where Audit_Status=1 AND Audit_Enter_uid = $user group by polyid");
		return $query->result();
	...
	function getLogData($cmd){
		...
		*/
		...
		$SQL ="SELECT count(am.assets_name) as total from ".$this->tbl_assets." am left join ".$this->tbl_current." lm on lm.device_id = am.device_id where am.status=1 AND am.del_date is null AND lm.zone_id >0";
		...
		{
			//$SQL .=" AND tm.device_id = '".$device."'";
			$SQL .=" AND find_in_set(lm.device_id,'$device')";
		}
		...
		$SQL = "SELECT am.assets_name, lm.current_area from ".$this->tbl_assets." am left join ".$this->tbl_current." lm on lm.device_id = am.device_id where am.status=1 AND am.del_date is null AND lm.area_id > 0";

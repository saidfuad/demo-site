...
function view_distance_vehicle_report_map(id){
	var nameToCheck = "<?php echo $this->lang->line("Distance Vehicle Report"); ?>";
	...
	$('#tabs ul.ui-tabs-nav li a').each(function(i) {
		if (this.text == nameToCheck) {
			...
			$('#tabs').tabs('add', 'reports/distance_vehicle_report/viewMap/0/id/'+id, '<?php echo $this->lang->line("Distance Vehicle Report"); ?>');
			return false;
		}
	});
	if (!tabNameExists){

...
jQuery().ready(function (){
	...
	jQuery("#allpoints_grid<?php echo time(); ?>").jqGrid({
		...
		datatype: "local",
		colNames:["<?php $this->lang->line("ID"); ?>",'<?php echo $this->lang->line("Datetime"); ?>','<?php echo $this->lang->line("Status"); ?>', '<?php echo $this->lang->line("Address"); ?>', '<?php echo $this->lang->line("Speed"); ?>', '<?php echo $this->lang->line("View_on_Map"); ?>'],
		colModel:[
			...
			{name:"status",editable:true, index:"status", width:80, align:"center", jsonmap:"status"},
...
/*	if(img == '<?php echo base_url(); ?>assets/marker-images/mini-RED-BLANK.png') {
	var new_marker = new google.maps.Marker({
		position: point,
...
function viewTrackAllpoint(devText){
	...
	//alert(latArr.length);
	totalDir=Math.floor(latArr.length/(skip_point+2));
...
function call_start_new_line(arr_i)
{
		...
		if(arr_i == 0){
		...
		else if(arr_i == (latArr.length-1) || (latArr.length - arr_i < (skip_point+2))){
			//alert("1-="+arr_i);
			...
			{
				for(xi=arr_i;xi<=arr_i+skip_point;xi++)
				{
					if(ignitionArr[xi]==0){
						alert('zero');
						img = "<?php echo base_url(); ?>assets/marker-images/kml-RED-END.png";
					}	
				}
			}
	...
	//alert("3->"+arr_i);
		...
		if(totalDir > totalDir_count)
		{
			...
			//comment rajdeep if(latArr.length != arr_i)
				//comment rajdeep setTimeout(function(){call_start_new_line(arr_i)},Timer_counter);
			totalDir_count++;
...
//			$("#all_point_pBar").progressbar("value" , 99.99);
			...
			$("#all_point_pBar").css("display","none");
				setTimeout(function(){
					...
					var txt = "<?php echo $this->lang->line("Distance"); ?> : " + distance_all_total.toFixed(2) + " <?php echo $this->lang->line("KM"); ?>";
...
function AllPointfilterAssetsCombo(val, cid){
	$.post("<?php echo base_url(); ?>index.php/home/filter_assets", { grp: val},
	 function(data) {
		$("#"+cid).html("<option value=''>Please Select</option>"+data);
...
<table border="5" width="100%" class="formtable" style="margin-bottom: 5px;">
	<tr>
		...
		<td width="20%"><?php echo $this->lang->line("Group"); ?> : <select onchange="AllPointfilterAssetsCombo(this.value,'all_points_device')" name="group" id="group_device_stopreport" class="select ui-widget-content ui-corner-all" ><?php echo $group1; ?></select></td>
		<td width="20%"><?php echo $this->lang->line("Group"); ?> : <select onchange="AllPointfilterAssetsCombo(this.value,'all_points_device')" name="group" id="group_device_stopreport" class="select ui-widget-content ui-corner-all" ><?php echo $group1; ?></select></td>
		<td width="20%"><?php echo $this->lang->line("Assets"); ?> : <select name="device" id="all_points_device" class="select ui-widget-content ui-corner-all"></select></td>
	</tr>
	<tr>

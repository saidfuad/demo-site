...
<?php
	...
	$("#inspection_sdate").datetimepicker('setDate', new Date());
	$("#inspection_edate").datetimepicker('setDate', new Date());
...
function viewLocationInspection(id){
	var nameToCheck = "<?php echo $this->lang->line("Inspection Report Map"); ?>";
	...
	$('#tabs ul.ui-tabs-nav li a').each(function(i){
		if (this.text == nameToCheck){
			...
			$('#tabs').tabs('add', 'reports/dealer_stopreport/view_map/0/id/'+id, '<?php echo $this->lang->line("Inspection Report Map"); ?>');
			return false;
		}
	});
	if (!tabNameExists){
		$('#tabs').tabs('add', 'reports/inspection/view_map/0/id/'+id, '<?php echo $this->lang->line("Inspection Report Map"); ?>');
	}
}
</script>
<?php

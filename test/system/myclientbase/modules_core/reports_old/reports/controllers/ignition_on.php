	...
	function view_map(){
		...
		if(count($rows)) {
			foreach ($rows as $row) {
				...
				$stp_html .= "<tr><td>".$this->lang->line("From").' : '."</td><td>".$ignition_off_date.' '.$ignition_off_time. "</tr></td>";
				$stp_html .= "<tr><td>".$this->lang->line("To").' : '."</td><td>".$ignition_on_date.' '.$ignition_on_time. "</tr></td>";
				$stp_html .= "<tr><td>".$this->lang->line("Duration").' : '."</td><td>".$row->duration. "</tr></td></table>";
				$data['html'] = $stp_html;
			} 
		}
		else{

...
function live_clearPolyOverlayszone<?php echo $prefix; ?>() {
  if (live_poly_zone_array<?php echo $prefix; ?>) {
    for (i in live_poly_zone_array<?php echo $prefix; ?>) {
      live_poly_zone_array<?php echo $prefix; ?>[i].setMap(null);
    }
  }
}
function live_showPolyOverlayszone<?php echo $prefix; ?>() {
 
  if (live_poly_zone_array<?php echo $prefix; ?>) {
    for (i in live_poly_zone_array<?php echo $prefix; ?>) {
	  live_poly_zone_array<?php echo $prefix; ?>[i].setMap(mapmap<?php echo $prefix; ?>);
    }
  }
}
...
function reload_Map<?php echo $prefix; ?>(){
	...
	//Trip Button
		...
		if(disp_zone<?php echo $time; ?>==true){
			var trackNewWindowDiv = document.createElement('DIV');
			var trackNewWindowControl = new zoneControlLive<?php echo $prefix; ?>(trackNewWindowDiv, mapmap<?php echo $prefix; ?>);
			trackNewWindowDiv.index = 1;	
			mapmap<?php echo $prefix; ?>.controls[google.maps.ControlPosition.TOP_RIGHT].push(trackNewWindowDiv);
		}
		<?php } ?>
        ...
        success: function(data) {
			...
			if(data){
				if(data.lat.length > 0) {
					...
					if(display_lines<?php echo $time; ?>) {
				...
				$("#loading_top").css("display","none");
				$("#history_<?php echo $prefix; ?>").html('Play Back More');
			}
	    }
    });
		...
		directionsService.route(request, function(response, status) 
		{
			if (status == google.maps.DirectionsStatus.OK) 
			{
				...
				{				
					...
					mapmap<?php echo $prefix; ?>.setCenter(pnt);
				}	
...
function zoneControlLive<?php echo $prefix; ?>(controlDiv, map){
  controlDiv.style.padding = '5px';	 
  // Set CSS for the control border
  var controlUI = document.createElement('DIV');
  controlUI.style.backgroundColor = 'white';
  ...
  controlUI.style.cursor = 'pointer';
  controlUI.style.textAlign = 'center';
  controlUI.title = 'Click to Show/Hide Zone';
  controlDiv.appendChild(controlUI);

  var chkOne = document.createElement( "input" );
  chkOne.type = "checkbox";
  chkOne.id = "chkZoneLive<?php echo $prefix; ?>";
  chkOne.checked = false;
  var lblOne = document.createElement('label')
  lblOne.htmlFor = "chkZoneLive<?php echo $prefix; ?>";
  ...
  controlUI.appendChild(lblOne);
  google.maps.event.addDomListener(controlUI, 'click', function() {
	if($("#chkZoneLive<?php echo $prefix; ?>").attr("checked") == "checked") {
		live_showPolyOverlayszone<?php echo $prefix; ?>();
		
	}else{
		live_clearPolyOverlayszone<?php echo $prefix; ?>();
	}
  });
}
...
function loadZones<?php echo $prefix; ?>(id){
	$("#pbar_Loading_<?php echo $prefix; ?>").html("Loading Zone... <span style='color:blue;cursor:pointer;' onClick='cancelLoading<?php echo $prefix; ?>();'>Cancel</span>");
	var label<?php echo $prefix; ?> = new Array();
	var live_polyV<?php echo $prefix; ?> = new Array();
	$loadingDynamicObj<?php echo $prefix; ?> = $.ajax({
        type: "GET",
        url: "<?php echo base_url(); ?>index.php/live/loadZone/id/"+id,
        dataType: "json",
        success: function(data) {
			for(i=0;i<data.pplyId.length;i++){
				var pId=data.pplyId[i];
				...
				live_poly_zone_array<?php echo $prefix; ?>.push(live_polyV<?php echo $prefix; ?>[i]);
				with ({ foo: i }){	
					google.maps.event.addListener(live_polyV<?php echo $prefix; ?>[foo],"mouseover",function(event){
						label<?php echo $prefix; ?>[foo].setMap(mapmap<?php echo $prefix; ?>);
			...
			mapmap<?php echo $prefix; ?>.controls[google.maps.ControlPosition.TOP_RIGHT].push(trackNewWindowDiv);
			<?php } ?>
			$("#pbar_Loading_<?php echo $prefix; ?>").html("Zone Loading Complete ...");
			contentLoadComplete();
		},
        error: function(request, status, err){
			$("#pbar_Loading_<?php echo $prefix; ?>").html("Zone Loading Canceled.");
			
			disp_zone<?php echo $time; ?>=false;
			contentLoadComplete();
        }
    });
}
			   ...
			   function(data){
					...
					setTimeout(function(){
						...
						setTimeout(function(){
							...
							loadZones<?php echo $prefix; ?>('<?php echo $prefix; ?>');
						},500);						
					},500);
				},'json');
			}else{

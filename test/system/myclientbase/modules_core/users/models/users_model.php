	...
	*/
		...
		for($js=0;$js<count($secondary_user);$js++)
		{
			...
			/*foreach ($query->result() as $row)
			{
			   $data_area = array( 'deviceid' =>  $row->deviceid,
				...
				$this->db->insert('areas', $data_area); 
			}*/
			
			$data_area= array();
			foreach ($query->result() as $row)
			{
				$data_area[] = '("'.$row->deviceid.'", "'.$row->polyid.'", "'.$row->polyname.'", "'.$row->color.'", "'.$row->lat.'", "'.$row->lng.'", "'.$row->pointid.'", "'.$row->in_alert.'", "'.$row->out_alert.'", "'.$row->speed_alert.'", "'.$row->speed_value.'", "'.$row->speed_unit.'", "'.$row->area_size.'", "'.$row->addressbook_ids.'", "'.date("Y-m-d H:i:s").'", "'.$ids.'", "1", "'.$row->Audit_Comment.'", "'.$row->email_alert.'", "'.$row->sms_alert.'", "'.$row->area_type_opt.'")';
			}
			mysql_query('INSERT INTO areas (deviceid, polyid,polyname,color,lat,lng,pointid,in_alert,out_alert,speed_alert,speed_value,speed_unit,area_size,addressbook_ids,Audit_Enter_Dt,Audit_Enter_uid,Audit_Status,Audit_Comment,email_alert,sms_alert,area_type_opt) VALUES '.implode(',', $data_area));
			...
			/*foreach ($query->result() as $row)
			{
			   $data_zone = array( 'deviceid' =>  $row->deviceid,
				...
				$this->db->insert('landmark_areas', $data_zone); 
			}*/
			
			$data_zone= array();
			foreach ($query->result() as $row)
			{
				$data_zone[] = '("'.$row->deviceid.'", "'.$row->polyid.'", "'.$row->polyname.'", "'.$row->color.'", "'.$row->lat.'", "'.$row->lng.'", "'.$row->pointid.'", "'.$row->in_alert.'", "'.$row->out_alert.'", "'.$row->speed_alert.'", "'.$row->speed_value.'", "'.$row->speed_unit.'", "'.$row->area_size.'", "'.$row->addressbook_ids.'", "'.date("Y-m-d H:i:s").'", "'.$ids.'", "1", "'.$row->Audit_Comment.'", "'.$row->email_alert.'", "'.$row->sms_alert.'", "'.$row->area_type_opt.'")';
			}
			mysql_query('INSERT INTO landmark_areas (deviceid, polyid,polyname,color,lat,lng,pointid,in_alert,out_alert,speed_alert,speed_value,speed_unit,area_size,addressbook_ids,Audit_Enter_Dt,Audit_Enter_uid,Audit_Status,Audit_Comment,email_alert,sms_alert,area_type_opt) VALUES '.implode(',', $data_zone));
			...
			/*foreach ($query->result() as $row)
			{
			   $data_route = array( 'routename' =>  $row->routename,
				...
				$this->db->insert('tbl_routes', $data_route); 
			}*/
			
			$data_route= array();
			foreach ($query->result() as $row)
			{
				$data_route[] = '("'.$row->routename.'", "'.$row->route_color.'", "'.$row->landmark_ids.'", "'.$row->start_point.'", "'.$row->end_point.'", "'.$row->waypoints.'", "'.$row->points.'", "'.$row->deviceid.'", "'.$row->distance_value.'", "'.$row->distance_unit.'", "'.$row->total_distance.'", "'.$row->total_time_in_minutes.'", "'.$row->round_trip.'", "'.$ids.'", "'.date("Y-m-d H:i:s").'", "'.$ids.'", "1", "'.$row->comments.'", "'.$row->email_alert.'", "'.$row->sms_alert.'")';
			}
			mysql_query('INSERT INTO tbl_routes (routename, route_color,landmark_ids,start_point,end_point,waypoints,points,deviceid,distance_value,distance_unit,total_distance,total_time_in_minutes,round_trip,userid,add_date,add_uid,status,comments,email_alert,sms_alert) VALUES '.implode(',', $data_route));
			...
			/*foreach ($query->result() as $row)
			{
			   $data_landmark = array( 'name' =>  $row->name,
				...
				$this->db->insert('landmark', $data_landmark); 
			}*/
			
			$data_landmark= array();
			foreach ($query->result() as $row)
			{
				$data_landmark[] = '("'.$row->name.'", "'.$row->address.'", "'.$row->radius.'", "'.$row->distance_unit.'", "'.$row->device_ids.'", "'.$row->icon_path.'", "'.$row->lat.'", "'.$row->lng.'", "'.$ids.'", "'.date("Y-m-d H:i:s").'", "1", "'.$row->comments.'", "'.$row->group_id.'", "'.$row->email_alert.'", "'.$row->sms_alert.'", "'.$row->alert_before_landmark.'")';
			}
			mysql_query('INSERT INTO landmark (name, address,radius,distance_unit,device_ids,icon_path,lat,lng,add_uid,add_date,status,comments,group_id,email_alert,sms_alert,alert_before_landmark) VALUES '.implode(',', $data_landmark));
			...
			foreach ($query->result() as $row)
			{
				$data_groups[] = '("'.$row->group_name.'", "'.$row->assets.'", "'.$ids.'", "'.date("Y-m-d H:i:s").'", "1", "'.$row->comments.'")';
			}
			mysql_query('INSERT INTO group_master (group_name, assets,add_uid,add_date,status,comments) VALUES '.implode(',', $data_groups));
		}
	}
	...
	function getAllData($cmd){
		...
		*/
		...
		$SQL = "SELECT us.user_id, us.username,mup.profile_name as profile_id, us.first_name, us.last_name, us.address, us.city, us.state, us.zip, us.country, us.phone_number, us.fax_number, us.mobile_number, us.email_address,  us.company_name, us.birth_date, us.report_view, us.menu_view, us.change_password, us.history, us.allow_user_profile, us.from_date, us.to_date, (select username from tbl_users where user_id=us.admin_id) as admin_id, us.status, mc.name as city,ms.name as country,mst.name as state FROM tbl_users as us left join mst_city as mc on mc.id = us.city left join mst_country as ms on ms.id = us.country left join  mst_user_profile as mup on mup.id = us.profile_id left join mst_state as mst on mst.id = us.state WHERE us.del_date is null";
		...
		if($cmd=="export")   
		{
			...
			$fitr .="<tr>";
			$fitr.="<th>Id</th>";
			$fitr.="<th>First Name</th>";
			$fitr.="<th>Last Name</th>";
			$fitr.="<th>Username</th>";
			$fitr.="<th>D.O.B</th>";
			$fitr.="<th>Profile</th>";
			$fitr.="<th>Address</th>";
			...
			$fitr.="<th>Email</th>";
			$fitr.="<th>Company Name</th>";
			$fitr.="<th>Valid From</th>";
			$fitr.="<th>Valid To</th>";
			$fitr.="<th>Status</th>";
			$fitr .="</tr>"; 
			//var_dump($result);
			...
			echo "<tr><th colspan='17'>Users</th></tr>";
			echo $fitr;
			echo $EXCEL;
			echo "</table>";
			die(); 
	...
	function save($db_array, $id=NULL, $set_flashdata = TRUE) {
		...
		unset($db_array['confirm_password']);

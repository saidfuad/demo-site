...
/*	
	tree.on('expand', function (event) {
		...
		var loader = false;
		var loaderItem = null;
		var loaderUrl = tree.jqxTree('getItem', event.args.element).value;
		...
		if (loader) {
			...
			$.ajax({
				url: loaderUrl, data: {'sel': tree_elements}, type: "POST",
	...
	tree.on('collapse',function (event) {
		...
		var $element = $(event.args.element);
		$element.find('ul:first')[0].innerHTML = "";
		var item = $('#jqxTree').jqxTree('getItem', args.element);		
		tree.jqxTree('addTo', {value: item.value, label: "Loading..." }, $element[0]);
	});
*/	
...
function loadVehicleDistanceData(){	
	...
	vDStartLatLong = [];
	vDEndLatLong = [];
	$.post("<?php echo base_url(); ?>index.php/tree/getDist/id/"+selected_assets_ids,function(data){
...
function initializeTree() {
	...
	tree.jqxTree({theme: 'ui-redmond', source: source, hasThreeStates: true, height: '95%', checkboxes: true});
...
?>
	...
	<div class="ui-layout-center" id="tabs" style="float:left;position:relative;">
			...
			<div id="tabs-1">
				...
				<div class="sixteen columns half-bottom" style="width:100% !important; float:left; margin-top: 20px;">
						...
						<input type='checkbox' onclick='stop_resume_toggle()' <?php if($auto_refresh_setting == 1) echo 'checked="checked"'; ?> id='checkboxToggle'> <?php echo $this->lang->line('data_refresh_after'); ?> <input type='text' size='2' onblur='counter_change()' value='15' id='time_in_seconds'> <?php echo $this->lang->line('seconds'); ?> (<?php echo $this->lang->line('refresh_after'); ?> <span id='seconds'>15</span> <?php echo $this->lang->line('second'); ?>) &nbsp;&nbsp;<span onClick="reloadDashboard_Assets_Timer()" style="font-weight:bold;text-decoration:underline;cursor:pointer"><?php echo $this->lang->line('refresh'); ?></span> <?php echo $this->lang->line('Select/Unselect All'); ?></div>
						</div -->                        
	...
	<div class="ui-layout-west" id="west_sub_menu">
     <!-- a id="jqxbutton" style="cursor:pointer;"><img src="<?php
     echo base_url(); ?>assets/images/map.png" style="margin-left:
     10px;" atl="Load on Map" height="32" width="32" /></a -->

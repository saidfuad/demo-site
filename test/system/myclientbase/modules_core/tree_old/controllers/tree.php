...
class Tree extends Admin_Controller {
	...
	function index(){
		...
		if(count($rows)) {
			foreach ($rows as $row) {
				...
				$groupList .= "<li><a href='alert(g-".$row->id.")'>".addslashes($row->group_name)."</a></li>";
			}
		}
		...
		if(count($rows)) {
			foreach ($rows as $row) {
				...
				$subUserList .= "<li><a href='alert(u-".$row->user_id.")'>".$row->username." (".addslashes($row->first_name)." ".addslashes($row->last_name).")</a></li>";
			}
		}
		...
		if(count($rows)) {
			foreach ($rows as $row) {
				...
				$areasList .= "<li><a href='alert(a-".$row->polyid.")'>".addslashes($row->polyname)."</a></li>";
			}
		}
		...
		if(count($rows)) {
			foreach ($rows as $row) {
				...
				$landList .= "<li><a href='alert(l-".$row->id.")'>".addslashes($row->name)."</a></li>";
			}
		}
		...
		foreach ($rows as $row)
		{
			...
			$ownerList .= "<li><a href='alert(o-".$row->id.")'>".addslashes($row->owner)."</a></li>";
		}
		...
		foreach ($rows as $row)
		{
			...
			$divisionList .= "<li><a href='alert(d-".$row->id.")'>".addslashes($row->division)."</a></li>";
		}
		...
		/*if($this->session->userdata('usertype_id')==3){
			$rows = $this->tree_model->get_userDisplaySetting($this->session->userdata('user_id'));
			$d_assets_cmb="";
			if(count($rows)>1)
				$d_assets_cmb.="<option value=''>Please Select</option>";
			if(count($rows)) {
	...
	function tree_list()
	{
		...
		if(trim($assets) != '') {
			$rows = $this->tree_model->treeLastLocation($assets);
		}
...
//				if($l >= $max) break;
		...
		if(count($rows) > 0) {
			foreach ($rows as $row) {
				...
				else if($row->beforeTime < $this->session->userdata('network_timeout')) {
					$color = 'green';
				}
				else {
					$color = 'red';
		...
		$data[] = array("label" => "Group", 'value' => 'groupList', 'items' => $groupListNew);
		$data[] = array("label" => "Landmark", 'value' => 'landmarkList', 'items' => $landListNew);
		$data[] = array("label" => "Area", 'value' => 'areaList', 'items' => $areasListNew);
		$data[] = array("label" => "Owner", 'value' => 'ownerList', 'items' => $ownerListNew);
		$data[] = array("label" => "Division", 'value' => 'divisionList', 'items' => $divisionListNew);
		if(count($allAssetsList)) {
			$data[] = array("label" => "Other Assets", 'value' => 'allAssetsList', 'items' => $allAssetsList);
		}
		
		$tree_data[] = array("label" => ucwords($this->session->userdata('first_name')), 'value' => $this->session->userdata('user_id'), 'expanded' => true);

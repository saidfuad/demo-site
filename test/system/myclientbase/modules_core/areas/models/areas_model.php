	...
	*/
	...
	function getAllData($cmd){
		...
		*/
		...
		$SQL = "SELECT count(*) as total FROM areas as ar where ar.Audit_Status=1 and ar.Audit_Del_Dt is null and ar.Audit_Enter_uid = ".$this->session->userdata('user_id');
		...
		*/
		...
		$SQL = "SELECT ar.id, (SELECT GROUP_CONCAT(`assets_name`) FROM assests_master WHERE find_in_set(id,ar.deviceid) and status='1') as deviceid, ar.polyid, ar.polyname, ar.color, ar.in_alert, ar.out_alert, (SELECT GROUP_CONCAT(`name`) FROM addressbook WHERE find_in_set(id,ar.addressbook_ids)) as address_book_nm, ar.sms_alert, ar.email_alert, ar.area_type_opt FROM areas ar where Audit_Status=1 and ar.Audit_Enter_uid = ".$this->session->userdata('user_id');
		if($where != "")
			$SQL .= " AND $where";
	...
	{
		$ids = $_POST["id"];
		$qry="SELECT group_concat(polyid) as polyids from areas where find_in_set(id,'".$ids."')";
		$rs=$this->db->query($qry);
		$polyids=$rs->result_array();
		$polyid=$polyids[0]['polyids'];
		$dt=gmdate("Y-m-d H:i:s");
		$tblUsr="UPDATE `areas` SET `Audit_Status`=0, `Audit_Del_Dt`='".$dt."', `Audit_Del_uid`=".$this->session->userdata('user_id')."  WHERE polyid in(".$polyid.")";

...
class Areas extends Admin_Controller {
	...
	function loadData($cmd='false'){		
		$data = $this->areas_model->getAllData($cmd); 
		$responce->page = $data['page'];
		$responce->total = $data['total_pages'];
		$responce->records = $data['count'];
		$i=0;
		foreach($data['result'] as $row) {
			$row->polyname = '<a style="cursor:pointer;" onclick="edit_in_map('.$row->polyid.')">'.$row->polyname.'</a>';
			$responce->rows[$i] = $row;
			$i++;
		}
	...
	function deleteData(){
		...
		$this->output->set_output($this->areas_model->delete_areas());
	}
	...
	function form() {
		...
		if (!$this->form_model->validate()) {
			...
			$rows = $this->areas_model->prepare_assets();
			$opt = '';
			...
			$rows = $this->areas_model->getAddressBookGroupList();
			...
			if(uri_assoc('id')){
				$this->areas_model->save($formdata, uri_assoc('id'));
			}
		}
	}
	function getIco()
	...
	function state_data(){
		...
		$this->output->set_output($query=$this->areas_model->state());
	}
	function city_data(){
		$this->load->model('areas_model','',TRUE);
		//echo $query=$this->areas_model->city();
		$this->output->set_output($query=$this->areas_model->city());
	}
	function get_json_data()
	{
		$this->load->model('areas_model','',TRUE);
		$rs=$this->areas_model->get_json();
		//echo json_encode($rs);
	...
	function check_duplicates()
	{
		...
		$id=uri_assoc("id");
		if(!$this->areas_model->checkUserDuplicate($username,$id))
		{

...
function loadAssets(){
	...
	if(tree_elements.length > 0) {
		...
		$req_obj_xhr = $.post(url, { assets:tree_elements, limit:limit, page:page }, function(data) {
			$("#main").html(data);
...
function reloadAssets(){
	...
	if(tree_elements.length > 0) {
		$("#loading_top").css("display","block");
		// , sel:selected_assets_ids
		if(url == "<?php echo base_url(); ?>index.php/tree/grid_view") {
			jQuery("#treelastpoint_grid").trigger("reloadGrid");
			$("#loading_top").css("display","none");
		}
		else {
			$.post(url, { assets:tree_elements, limit:limit, page:page }, function(data) {
				$("#loading_top").css("display","none");
				$("#main").html(data);
			});
		}
...
function currentSelectedAssets() {
	selected_assets_ids = $("#main input:checked").map(function () {
		return this.value;
	}).get().join(",");
}

...
$reload = base_url() . "index.php/tree/assets"; 
if($page<=0)  $page  = 1;
...
<style>
	...
	<div style="font-size: 12px;font-weight: bold; margin:5px; padding-bottom: 10px;"><?php echo $this->lang->line('Last Data Recieved'); ?> : <?php echo date("$date_format $time_format"); ?> </div>
	<div style="clear:both"></div>
	...
	?> 
	<ul class="alist" style="width:100%;">
...
*/			
			...
			if($seconds_before > ($this->session->userdata('network_timeout')/60)){
				$clr = "orange";
			}else if($coord->ignition == 0 && $coord->speed == 0){
				$clr = "#06F";
			}else{
				$clr = "green";
			}
	...
	?>
		...
		<?php /* if(! in_array($coord->assets_id, $selected)) { ?>
				dashboardMarkers[<?php echo $coord->assets_id; ?>].setMap(null);
				dLabelArr[<?php echo $coord->assets_id ?>].setMap(null);
		<?php } */ ?>
	...
	//	if($coord->speed >= $coord->old_speed ){ $disp_icon="SpeedUp.png"; }else{ $disp_icon="SpeedDown.png";}
		...
		<li style="display:inline-block;white-space:nowrap;float:none;"><div class="deviceMain <?php echo $class; ?>">
			...
			<?php } ?>
			<!-- input onclick="selectedAssets(<?php echo $coord->assets_id; ?>);" name="assets_check[]" value="<?php echo $coord->assets_id; ?>" type="checkbox" <?php echo (in_array($coord->assets_id, $selected) ? 'checked="checked"' : ''); ?> class="asset_checkbox <?php echo $ast_class; ?>" style='padding: 0px; margin: 0px;'/ --> <span style="color: #FFFFFF;"><?php if($coord->speed=='0') echo '00'; else if($coord->speed!="" || $coord->speed!=null) echo $coord->speed; else echo "00";?></span></div>
	...
	<!-- div id="distanceBtn_div_kunal" style="display:none;-webkit-box-shadow:0px 0px 12px rgba(0, 0, 0,0.4);-moz-box-shadow: 0px 1px 6px rgba(23, 69, 88, .5);">

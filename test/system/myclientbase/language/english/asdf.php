	...
	*/
	...
	//	$cmd=uri_assoc('cmd');
		...
		*/
		...
		$SQL = "SELECT dm.id,CONCAT(am.assets_name,'(',am.device_id,')') as assets_id, am.id as aId, dm.add_date as add_date, dm.distance, dm.running_time from distance_master dm left join assests_master am on am.id=dm.assets_id WHERE ";
		if($this->session->userdata("usertype_id")!=1){
			$SQL .=  " am.id in (select id from assests_master where find_in_set(id, (SELECT assets_ids FROM user_assets_map where user_id = $user))) and ";
		}
		$SQL .= " dm.add_date BETWEEN '" . $sdate . "' AND '" . $edate . "'";

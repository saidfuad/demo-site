...
class Device_model extends Model 
{
	...
	*/
	...
	{
		$id = uri_assoc('id');
                //Added By Poonam 21-1-2015 7:59 PM(In below Query del_uid=NULL and status=1 is added)
		$qry=$this->db->query("select tr.landmark_ids as landmarks, am.assets_name from assests_master am left join tbl_routes tr on am.current_trip=tr.id where am.device_id=$id and am.current_trip is not null and tr.del_uid is NULL and tr.status=1");

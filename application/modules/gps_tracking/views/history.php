...
<script type="text/javascript">
    ...
    function calcRoute(startLat, startLng, stopLatLng, endLat, endLng) {
        if (timerHandle) {
        ...
        directionsDisplay = new google.maps.DirectionsRenderer(rendererOptions);
        var start = new google.maps.LatLng(parseFloat(startLat), parseFloat(startLng));
        var end = new google.maps.LatLng(parseFloat(endLat), parseFloat(endLng));
        console.log("startEnd: ", parseFloat(startLat), parseFloat(startLng), parseFloat(endLat), parseFloat(endLng))
        var travelMode = google.maps.DirectionsTravelMode.DRIVING
        var waypts = [];
        for (var i = 0; i < stopLatLng.length; i++) {
            console.log("stopPoints: ", parseFloat(stopLatLng[i].lat), parseFloat(stopLatLng[i].lng));
            stop = new google.maps.LatLng(parseFloat(stopLatLng[i].lat), parseFloat(stopLatLng[i].lng));
            waypts.push({
                location: stop
                , stopover: true
            });

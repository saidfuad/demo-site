...
class Mdl_gps_tracking extends CI_Model {
    ...
    function get_gps_vehicles($account_id = null, $query = null) {
        ...
        if($this->session->userdata('hawk_user_type_id') ==='7'){
            $this->db->where('vehicles.vehicle_type_id',7);
        }
...
function count_vehicles($account_id, $user_id = null )
{
    $this->db->select(' SUM(CASE  WHEN ignition = 1  AND speed > 0 THEN 1 ELSE 0 END) AS moving ,
    SUM(CASE  WHEN ignition = 1  AND speed = 0 THEN 1 ELSE 0 END) AS idle,
   SUM( CASE  WHEN ignition = 0  AND speed = 0 THEN 1 ELSE 0 END) AS parked,
   SUM( CASE  WHEN active_status = 0   THEN 1 ELSE 0 END) AS disabled,
   SUM(CASE  WHEN (speed_alert = 1  OR  arm_alert = 1 OR  power_cut = 1) THEN 1 ELSE 0 END) AS alert , ');
       $this->db->from('vehicles');
        $this->db->where('vehicles.account_id', $account_id);
   ...
   return $this->db->get()->row_array();
    ...
    function count_moving_vehicles($account_id) {
    ...
    function count_idle_vehicles($account_id) {
        $this->db->select('vehicles.vehicle_id');
        $this->db->from('vehicles');
        $this->db->where('vehicles.device_id!=', 0);
        $this->db->where('vehicles.speed', 0);
    ...
    function count_parked_vehicles($account_id) {
        $this->db->select('vehicles.vehicle_id');
        $this->db->from('vehicles');
        $this->db->where('vehicles.device_id!=', 0);
        $this->db->where('vehicles.speed', 0);
    ...
    function count_alert_vehicles($account_id) {
        $this->db->select('vehicles.vehicle_id');
        $this->db->from('vehicles');
        $this->db->where('vehicles.device_id!=', 0);
    ...
    function count_disabled_vehicles($account_id) {
        $this->db->select('vehicles.vehicle_id');
        $this->db->from('vehicles');
...
//    function get_routes($account_id = null) {
//        if ($account_id != null) {
//            $this->db->where('account_id', $account_id);
//        }
//

...
<?php if (sizeof($vehicles) == 0 && $this->session->userdata('hawk_user_type_id') != '7') { ?>
    ...
    <div class="no-vehicle-overlay">
        ...
        <div class="col-sm-5 col-md-5 bg-crumb no-vehicle-title" align="center">
            ...
            <br>
            <a class="btn btn-success" href="<?php echo base_url("index.php/vehicles/add_vehicle") ?>"><span class="fa fa-car fa-fw"></span>&nbsp; Add Vehicles</a><br>
            <br>
        </div>
...
<?php } else { ?>
    ...
    <div class="vehicle-details">
        ...
        <div id="hawk-image"><span></span></div>
        <div id="plate-no">Plate No: <span></span></div>
        <div id="model">Vehicle Model: <span></span></div>
        <div id="status">Status: <span></span></div>
        <div id="speed">Last Speed: <span></span></div>
        <div id="last-location">Last Location: <span></span></div>
        <div id="space"></div>
        <div id="button-details"><a class="btn btn-xs btn-primary" href="">View Vehicle Details</a></div>
    </div>
...
<div class="container-fluid fleet-view">
    <div class="row" style="margin-top: -64px">
       ...
       <!--         <div data-toggle='tooltip' data-original-title='Click to show a list of all of your vehicles. Toggle to hide.' data-placement='right' class="controls" id="map-vehicle-zone">
                ...
                <div id="filter-by-sacco">
                    <select class="form-control" id="filter-sacco">
                        <option value="null">All Saccos</option>
                        <?php foreach($saccos as $sacco){ ?>
                        <option value="<?php echo $sacco->user_id; ?>"><?php echo $sacco->company_name; ?></option>
                        <?php } ?>
                    </select>
                </div>
    ...
    $(function () {
        ...
        function refresh_vehicle_locations(times) {
            ...
            $.ajax({
                ...
                , url: "<?php echo base_url('index.php/ntsa/refresh_grid') ?>"
        ...
        function displayMarkers(res)
        {
            ...
            $("#badge1").load(base_url + "index.php/gps_tracking #badge1");
            $("#badge2").load(base_url + "index.php/gps_tracking #badge2");
            $("#badge3").load(base_url + "index.php/gps_tracking #badge3");
            $("#badge4").load(base_url + "index.php/gps_tracking #badge4");
            $("#badge5").load(base_url + "index.php/gps_tracking #badge5");


            var markers = res.map(function(res, i) {
                ...
                $marker_type = 'vehicle';
                $fetch_vehicle_url = "<?php echo site_url('ntsa/fetch_vehicle'); ?>";
                $fetch_vehicle_url = $fetch_vehicle_url + "/" + res.vehicle_id;
                $vehicle_history_url = " <?php echo site_url('ntsa/view_history'); ?>";
                $vehicle_history_url = $vehicle_history_url + "/" + res.vehicle_id;
                //  $title = res.plate_no;
                $title = '<br><div style="text-align: left"><b>Plate No.:</b>' + '<div style="float: right">' + res.plate_no + '</div></div>' +
                        '<div style="text-align: left"><b>Address: &nbsp; &nbsp; </b>' + '<div style="float: right">' + res.address + '</div></div>' +
                        '<div style="text-align: left"><b>Last seen:</b>' + '<div style="float: right">' + formatDate(res.last_seen) + '</div></div>' +
                        '<br><div style="text-align: left"><a class="btn btn-xs btn-primary" href=' + $fetch_vehicle_url + '> View Details</a> <div style="float: right"> <a class="btn btn-xs btn-primary" href=' + $vehicle_history_url + '>View History</a> </button></div> </div>';
                $plate_no = res.plate_no;
                return addMarker($marker_type, $title, $position, $icon, $fill_color, $vehicle_id, $plate_no);
        ...
        function addMarker(type, title, location, image, fill_col, vehicle_id, plate_number) {
        ...
        $('#filter-sacco').change(function(){
            $('#vehicle-search').val('');
            var sacco_id = $(this).val();
      
            filter_grid(sacco_id);
        });

        function filter_grid(sacco_id) {
            ...
            $.ajax({
                ...
                url: "<?php echo base_url('index.php/ntsa/filter_grid') ?>",
                success: function (response) {
                    res = JSON.parse(response);
                    displayMarkers(res);
                }

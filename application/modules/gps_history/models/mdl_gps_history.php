...
class Mdl_gps_history extends CI_Model {
    ...
    function get_vehicle_history($vehicle_id, $start_date=null, $stop_date=null)
    {
        ...
        $date   = time() - 60*60*1;
        $SQL = "select latitude,longitude,tracking_time from gps_track_points";
  

        if($start_date!= null)
        {
            $start_date =date("Y-m-d H:i", strtotime($start_date));
           $SQL = $SQL." where tracking_time >='" . $start_date."'";
        }
        else{
            
            $SQL =  $SQL." where tracking_time >='" . date("Y-m-d H:i:s",$date) . "' AND vehicle_id = '" . $vehicle_id."' ORDER BY tracking_time ASC " ;
        ...
        if($stop_date != null)
        {
           $stop_date = date("Y-m-d H:i", strtotime($stop_date));
            $SQL = $SQL." AND tracking_time <='" . $stop_date."'";
        }

        $SQL = $SQL." AND vehicle_id = '" . $vehicle_id."' ORDER BY tracking_time ASC " ;
        $query = $this->db->query($SQL);
        return $query->result();
    }

...
class Gmap {
    ...
    function getHeaderJS() {
		...
		$_headerJS .= "<script type='text/javascript' src='http://maps.googleapis.com/maps/api/js?libraries=places&sensor=".(($this->mobile==true)?"true":"false")."'></script>";
		
		$_headerJS .= "<script type='text/javascript' src='".base_url()."assets/javascript/keydragzoom_packed.js'></script>";
	...
	*/
     ...
     */
    function getOnLoad() {
        return '<script language="javascript" type="text/javascript">window.onload=onLoad'.$this->map_id.';</script>';
    }
        ...
        */
		...
		if($this->display_map){
			...
			$_script .= "
				...
				google.maps.event.trigger(map$_key, 'resize');
			";
    ...
    function getNewMapJS() {
        ...
        $_output .= "/**************************************************\n";
        $_output .= " * Created with GoogleMapAPI" . $this->_version . "\n";
        $_output .= " * Author: Soni Kunal <sonikunal.4u@gmail.com>\n";
        $_output .= " * Link http://code.google.com/p/phpgooglemapapiv3/\n";
        $_output .= " * Copyright 2010 Soni Kunal\n";
        $_output .= " *************************************************/\n";
		if($this->street_view_dom_id!=""){
			$_script .= "
				var panorama".$this->street_view_dom_id."$_key = '';
     ...
     */
    function getAddMarkersJS($map_id = "map", $pano= false) {
		//defaults
		if($map_id == ""){
			$map_id = $this->map_id;
		}
